...
def _parse_old_causeway_contract():
...
class TestOldCausewayContract(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        (
            cls.room_ids,
            ...
            for objective in quest.get("objectives", []):
                ...
                if obj_type == "deliver":
                    ...
                    self.assertIn(
                        flagged_drop,
                        self.item_ids,
                        f"{quest['quest_id']} delivery item {flagged_drop} is not defined in old_causeway",
                    )
        ...
        for quest in self.quests:
            for objective in quest.get("objectives", []):
                ...
                self.assertNotEqual(
                    ...
                    f"{quest['quest_id']} should deliver across Causeway subregions, not within {giver_room}",
                )
    ...
    def test_waystation_vendor_uses_curated_stock(self):
        self.assertIn("npc_waystation_host_tam", self.vendor_configs)

        stock = self.vendor_configs["npc_waystation_host_tam"].get("vendor_item_ids") or []
        accepts = set(self.vendor_configs["npc_waystation_host_tam"].get("vendor_accepts") or [])

        self.assertTrue(stock, "npc_waystation_host_tam should define a curated vendor_item_ids list")
        self.assertEqual({"consumable"}, accepts)

        for item_id in stock:
            self.assertIn(item_id, CATALOG, f"npc_waystation_host_tam references missing catalog item {item_id}")
            item_def = CATALOG[item_id]
            self.assertFalse(
                item_def.get("is_quest_item"),
                f"npc_waystation_host_tam should not stock quest item {item_id}",
            )
            self.assertIn(
                item_def.get("item_type"),
                accepts,
                f"npc_waystation_host_tam stocks {item_id} without accepting its item_type",
            )
            self.assertNotEqual(
                item_def.get("rarity"),
                "rare",
                f"npc_waystation_host_tam should not stock rare endgame gear on the Old Causeway",
            )

    def test_waystation_vendor_covers_field_needs(self):
        stocked = set()
