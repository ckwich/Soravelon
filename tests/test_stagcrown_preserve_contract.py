...
ZONE_PATH = pathlib.Path(__file__).resolve().parents[1] / "world" / "areas" / "stagcrown_preserve.py"
...
def _resolve_room_ref(node, room_var_to_id):
    ...
    if isinstance(node, ast.Constant) and isinstance(node.value, str):
        return node.value
    raise AssertionError(f"Unsupported room reference in stagcrown_preserve.py: {ast.dump(node)}")


def _parse_stagcrown_contract():
...
class TestStagcrownPreserveContract(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        (
            cls.room_ids,
            ...
            for objective in quest.get("objectives", []):
                ...
                if obj_type == "deliver":
                    ...
                    self.assertIn(
                        flagged_drop,
                        self.item_ids,
                        f"{quest['quest_id']} delivery item {flagged_drop} is not defined in stagcrown_preserve",
                    )
        ...
        for quest in self.quests:
            for objective in quest.get("objectives", []):
                ...
                self.assertNotEqual(
                    ...
                    f"{quest['quest_id']} should deliver across Preserve subregions, not within {giver_room}",
                )
    ...
    def test_quartermaster_uses_curated_stock(self):
        self.assertIn("npc_keeper_quartermaster_tovin", self.vendor_configs)

        stock = self.vendor_configs["npc_keeper_quartermaster_tovin"].get("vendor_item_ids") or []
        accepts = set(self.vendor_configs["npc_keeper_quartermaster_tovin"].get("vendor_accepts") or [])

        self.assertTrue(stock, "npc_keeper_quartermaster_tovin should define a curated vendor_item_ids list")
        self.assertEqual({"consumable", "tool"}, accepts)

        for item_id in stock:
            self.assertIn(item_id, CATALOG, f"npc_keeper_quartermaster_tovin references missing catalog item {item_id}")
            item_def = CATALOG[item_id]
            self.assertFalse(
                item_def.get("is_quest_item"),
                f"npc_keeper_quartermaster_tovin should not stock quest item {item_id}",
            )
            self.assertIn(
                item_def.get("item_type"),
                accepts,
                f"npc_keeper_quartermaster_tovin stocks {item_id} without accepting its item_type",
            )
            self.assertNotEqual(
                item_def.get("rarity"),
                "rare",
                f"npc_keeper_quartermaster_tovin should not stock rare endgame gear in Stagcrown Preserve",
            )

    def test_quartermaster_covers_hunt_and_lake_needs(self):
        stocked = set()
