...
class TestStagcrownPreserveLayout(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.room_ids, cls.edges = _parse_stagcrown_layout()
    ...
    def test_zone_meets_room_budget(self):
        ...
        self.assertGreaterEqual(region_counts["fg"], 18)
        self.assertGreaterEqual(region_counts["mt"], 24)
        self.assertGreaterEqual(region_counts["lg"], 18)
        self.assertGreaterEqual(region_counts["pb"], 20)
        self.assertGreaterEqual(region_counts["lb"], 18)
        self.assertGreaterEqual(region_counts["sw"], 20)
    ...
    def test_cross_zone_exit_to_varath_prime_exists(self):
        self.assertIn(
            ("fg_charter_gate", "varath_prime:nh_preserve_gate", "south", False),
            self.edges,
        )

    def test_all_local_rooms_reachable_from_charter_gate(self):
        ...
        seen = {"fg_charter_gate"}
        queue = deque(["fg_charter_gate"])
    ...
    def test_hidden_routes_exist(self):
        self.assertIn(
            ("lg_hidden_postern", "pb_snare_run", "down", True),
            self.edges,
        )
        self.assertIn(
            ("lb_hidden_cove", "sw_hidden_spring", "down", True),
            self.edges,
        )
