...
Uses unittest.TestCase + MagicMock — pure logic tests, no Evennia DB needed.
"""
...
class TestDomainChannel(unittest.TestCase):
    ...
    def test_domain_channel_checks_membership(self):
        """DomainChannel.at_pre_channel_msg rejects non-members."""
        from typeclasses.channels import DomainChannel

        ch = DomainChannel.__new__(DomainChannel)
        ch.key = "combat"
        ch.db = _MockDB(domain_name="combat")
        ...
        result = ch.at_pre_channel_msg(message)
    ...
    def test_domain_channel_allows_member(self):
        ...
        # Need to mock super() call — at_pre_channel_msg returns True from parent
        with patch.object(DomainChannel.__bases__[0], "at_pre_channel_msg", return_value=True):
            result = ch.at_pre_channel_msg(message)
