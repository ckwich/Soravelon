...
def _parse_crownroad_north_contract():
...
class TestCrownroadNorthContract(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        (
            cls.room_ids,
            ...
            cls.spawn_mob_ids,
            cls.named_mob_ids,
        ) = _parse_crownroad_north_contract()
            ...
            for objective in quest.get("objectives", []):
                ...
                if obj_type == "deliver":
                    ...
                    self.assertIn(
                        flagged_drop,
                        self.item_ids,
                        f"{quest['quest_id']} delivery item {flagged_drop} is not defined in crownroad_north",
                    )
        ...
        for quest in self.quests:
            for objective in quest.get("objectives", []):
                ...
                self.assertNotEqual(
                    ...
                    f"{quest['quest_id']} should deliver across Crownroad subregions, not within {giver_room}",
                )
    ...
    def test_relay_vendor_uses_curated_stock(self):
        self.assertIn("npc_hostel_keeper_marro", self.vendor_configs)

        stock = self.vendor_configs["npc_hostel_keeper_marro"].get("vendor_item_ids") or []
        accepts = set(self.vendor_configs["npc_hostel_keeper_marro"].get("vendor_accepts") or [])

        self.assertTrue(stock, "npc_hostel_keeper_marro should define a curated vendor_item_ids list")
        self.assertEqual({"consumable"}, accepts)

        for item_id in stock:
            self.assertIn(item_id, CATALOG, f"npc_hostel_keeper_marro references missing catalog item {item_id}")
            item_def = CATALOG[item_id]
            self.assertFalse(
                item_def.get("is_quest_item"),
                f"npc_hostel_keeper_marro should not stock quest item {item_id}",
            )
            self.assertIn(
                item_def.get("item_type"),
                accepts,
                f"npc_hostel_keeper_marro stocks {item_id} without accepting its item_type",
            )
            self.assertNotEqual(
                item_def.get("rarity"),
                "rare",
                f"npc_hostel_keeper_marro should not stock rare endgame gear on the Crownroad",
            )

    def test_crownroad_vendor_covers_travel_needs(self):
        stocked = set()
