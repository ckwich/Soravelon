...
def build():
    ...
    bc_dust_tone_14 = area.room('bc_dust_tone_14', name='Water Overtone', desc='The last bell’s note carries a faint wavering overtone from the cistern passages ahead. Damp and waterline readings share its slate, connecting a changed sound to the practical life below before reaching for stranger explanations.', room_type='underground', indoor=True)
    ...
    dl_deep_listening_chamber = area.room('dl_deep_listening_chamber', name='Deep Listening Chamber', desc='Stepped seats face a stone sounding wall beneath notation that preserves blank spaces as carefully as measured tones. A bell echo moves through the chamber while loose coldvein stone and resonance shards are catalogued away from the old surfaces.', room_type='underground', indoor=True)
    dl_last_measure = area.room('dl_last_measure', name='Last Measure', desc='The final approved measuring station stands beside a listening frame that has broken free of its indexed mount. Its earlier readings remain useful, but warning cords make the instrument’s present motion part of the hazard record.', room_type='underground', indoor=True)
    dl_slow_bell_03 = area.room('dl_slow_bell_03', name='Long-Decay Bell', desc='A heavy bell releases one low tone that takes many breaths to fade. Observers record when they stop hearing it rather than agreeing on a ceremonial answer, preserving the differences between bodies, positions, and days.', room_type='underground', indoor=True)
    dl_stone_breath_04 = area.room('dl_stone_breath_04', name='Draft Aperture', desc='Cool air pulses through a narrow natural opening as distant pressure changes. A loose oreling scrapes mineral crust from its edge, giving some irregular sounds an immediate cause while leaving the changing draft unexplained.', room_type='underground', indoor=True)
    dl_kept_question_05 = area.room('dl_kept_question_05', name='Question Ledger', desc='A stone desk holds questions with their asker, evidence, failed tests, and reasons for postponement. Several have survived generations of new handwriting, important enough to keep and insufficiently grounded to answer.', room_type='underground', indoor=True)
    dl_deep_chamber_06 = area.room('dl_deep_chamber_06', name='Occupied Silence', desc='Seat marks and lamp soot show that people return to this chamber even when no unusual tone appears. Attendance records include maintenance watches and family vigils, allowing silence to remain part of civic life without becoming proof.', room_type='underground', indoor=True)
    dl_last_measure_07 = area.room('dl_last_measure_07', name='Retired Scale', desc='An old distance scale ends where newer surveys discovered its anchor had moved. The measurements are retained with a correction table, demonstrating how a trusted instrument can become historical evidence instead of current authority.', room_type='underground', indoor=True)
    dl_slow_bell_08 = area.room('dl_slow_bell_08', name='Weather Bell', desc='This slow bell is tested only after surface weather and cistern readings are posted beside it. Years of records expose correlations, exceptions, and missing days without turning any one pattern into a promised cause.', room_type='underground', indoor=True)
    dl_stone_breath_09 = area.room('dl_stone_breath_09', name='Condensation Pulse', desc='Moisture gathers and dries in bands around a cold seam as air moves through it. Removable papers record timing and mineral residue, leaving the stone untouched and the familiar phrase stone breath clearly attached to observed airflow.', room_type='underground', indoor=True)
    dl_kept_question_10 = area.room('dl_kept_question_10', name='Disputed Reading', desc='Three careful copies of the same tone chart disagree at one interval. None has been discarded; later listeners annotate position, fatigue, and instrument condition beside each, making disagreement a durable invitation to return.', room_type='underground', indoor=True)
    dl_deep_chamber_11 = area.room('dl_deep_chamber_11', name='Repair Listening', desc='A repaired ceiling brace carries small witness bells on either side of its join. Their logs track the structure’s behavior while occupied, connecting deep study to the unromantic responsibility of keeping the chamber safe.', room_type='underground', indoor=True)
    dl_last_measure_12 = area.room('dl_last_measure_12', name='Boundary of Measure', desc='Beyond a brass floor line, the wall offers no safe anchor for approved instruments. Sketches and distant observations continue, but the ledger names this physical limit rather than filling the inaccessible space with certainty.', room_type='underground', indoor=True)
    dl_slow_bell_13 = area.room('dl_slow_bell_13', name='Unanswered Bell', desc='A small bell hangs beside several dates when its expected return tone did not arrive. Maintenance, weather, and witness checks are complete; the empty result remains recorded without being promoted into revelation.', room_type='underground', indoor=True)
    dl_stone_breath_14 = area.room('dl_stone_breath_14', name='Return Station', desc='The deepest maintained point contains air gauges, spare light, a name-cord register, and a clear route back. Its closing slate asks what changed, what did not, and what remains unknown, turning departure into the next study’s beginning.', room_type='underground', indoor=True)
