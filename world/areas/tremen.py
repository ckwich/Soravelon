...
def build():
    ...
    ug_underhall_gate = area.room('ug_underhall_gate', name='Underhall Gate', desc='A stone gate frames the descent beneath Tremen, its doors standing open beneath a row of witness lamps. Archivist Belru keeps the departure ledger here and asks for purpose, companions, and expected return before discussing anything below.', room_type='path', indoor=True)
    ug_oath_lintel = area.room('ug_oath_lintel', name='Oath Lintel', desc='Names of families, crafts, and rescues cover the low lintel in tightly fitted bands. The oath beneath them asks visitors to carry names upward, disturb no sealed stone, and report damage before taking credit for discovery.', room_type='path', indoor=True)
    ug_cistern_key_post = area.room('ug_cistern_key_post', name='Cistern Key Post', desc='Keys, water measures, and cistern maps hang from a post beside a damp service channel. Bellcap mushrooms push through the oldest mortar, harvested around marked clusters that keep the stone cool and intact.', room_type='path', indoor=True)
    ug_stone_witness_04 = area.room('ug_stone_witness_04', name='Witness Stone', desc='A smooth block accepts chalk names from every group passing toward the descent. Returning hands circle their own marks and add any companion met below, turning a temporary list into a shared account.', room_type='path', indoor=True)
    ug_underhall_warning_05 = area.room('ug_underhall_warning_05', name='Air Warning', desc='Colored cloth strips hang across vents at several heights, showing the direction and strength of air from below. The darkest strip has gone slack, and Belru’s fresh note advises shorter watches beyond the next stair.', room_type='path', indoor=True)
    ug_locked_descent_06 = area.room('ug_locked_descent_06', name='First Seal', desc='An old locking wheel rests open against a door thick enough to outlast its hinges. New inspection wax bridges two hairline cracks, making any movement visible to the next person who checks the seal.', room_type='path', indoor=True)
    ug_family_mark_07 = area.room('ug_family_mark_07', name='Stonekin Mark', desc='A chisel mark shaped like joined hands identifies stone tended by Belru’s family line. Smaller marks around it record repairs by unrelated crews, proof that stewardship here can be inherited through work as well as blood.', room_type='path', indoor=True)
    ug_old_air_08 = area.room('ug_old_air_08', name='Air Well', desc='A narrow shaft draws mineral-cold air from deeper chambers and carries the smell of wet stone into the gatehouse. Cups of water on its rim make faint ripples whenever distant movement changes the draft.', room_type='path', indoor=True)
    ug_stone_witness_09 = area.room('ug_stone_witness_09', name='Return Stone', desc='Tokens left by descending groups occupy shallow hollows in a waist-high stone. Belru returns each token to its owner on arrival, while the few that remain gather dust beneath their recorded names.', room_type='path', indoor=True)
    ug_underhall_warning_10 = area.room('ug_underhall_warning_10', name='Name Warning', desc='A carved warning lists sealed family chambers by name rather than threat. Fresh chalk translates damaged lines for guests, ending with a plain request: if the name cannot be carried home, leave the stone where it is.', room_type='path', indoor=True)
    ug_locked_descent_11 = area.room('ug_locked_descent_11', name='Lower Lock', desc='Three massive bars have been drawn back from the lower door and secured in floor cradles. Their inspection dates are current, and a basket nearby holds wedges for keeping the passage safely open during measured work.', room_type='path', indoor=True)
    ug_family_mark_12 = area.room('ug_family_mark_12', name='Family Ledger', desc='A stone-leaf ledger maps family chambers, public passages, waterworks, and places whose ownership remains disputed. Corrections are signed by multiple witnesses, preventing one confident voice from quietly redrawing the underhalls.', room_type='path', indoor=True)
    ug_old_air_13 = area.room('ug_old_air_13', name='Draft Test', desc='Oil lamps stand at knee, chest, and head height beside a final bend in the descent. Their flames lean by different degrees, and a slate records the safe reading without pretending old air will stay obedient.', room_type='path', indoor=True)
    ug_stone_witness_14 = area.room('ug_stone_witness_14', name='Last Witness', desc='The final stone before the underhalls bears no names, only a shallow groove for the measure cord Belru sends below. Knots already tied into the cord record prior descents, each one a promise that evidence and people return together.', room_type='path', indoor=True)
    hl_high_lift = area.room('hl_high_lift', name='High Lift', desc='A timber platform hangs above the upper terraces on paired cables disappearing into cloud. Lift Forewoman Kelda inspects every fastening herself, then reads the weather board before allowing load or traveler onto the deck.', room_type='path', indoor=False)
    hl_cable_house = area.room('hl_cable_house', name='Cable House', desc='Winch drums, brake shoes, and carefully greased cable fill a stone house open to the lift court. Inspection tags hang from every moving part, including a fresh red tag that keeps one auxiliary drum out of service.', room_type='path', indoor=False)
    hl_sky_waiting_bench = area.room('hl_sky_waiting_bench', name='Sky Waiting Bench', desc='A long sheltered bench faces both the lift and the dragon courier perch above it. Departure slates change with wind and cable strain, while waiting travelers trade ridge news instead of pretending impatience can clear the sky.', room_type='path', indoor=False)
    hl_thin_air_token_04 = area.room('hl_thin_air_token_04', name='Breath Token', desc='Clay tokens stamped with measured breaths hang beside the ascent roster. Anyone struggling to finish the simple count is directed toward the lower acclimation rail before taking the high road.', room_type='path', indoor=False)
    hl_ridge_report_05 = area.room('hl_ridge_report_05', name='Ridge Dispatch', desc='A covered board receives reports from patrols, couriers, and lift crews arriving by different routes. Their times and weather notes overlap here, letting Kelda distinguish a local cable problem from a storm crossing the whole range.', room_type='path', indoor=False)
    hl_high_cable_06 = area.room('hl_high_cable_06', name='Main Cable Span', desc='The main cable leaves the court over a stone saddle polished by controlled movement. Fine wire whiskers along one section have been bound for replacement, and a chalk arrow ensures every inspector finds the flaw.', room_type='path', indoor=False)
    hl_weather_pause_07 = area.room('hl_weather_pause_07', name='Hold Board', desc='Lift, courier, and foot-route holds occupy separate columns on a black board. Each closure names the observed condition and the next review time, turning delay into a decision that can be checked and changed.', room_type='path', indoor=False)
    hl_lift_chant_08 = area.room('hl_lift_chant_08', name='Haul Cadence', desc='Painted marks along the winch rail divide the hauling call into steady breaths. Crews answer the lead voice together, and any missed response stops the drum before uneven strain reaches the cable.', room_type='path', indoor=False)
    hl_thin_air_token_09 = area.room('hl_thin_air_token_09', name='Acclimation Rail', desc='A gently rising rail gives high-road travelers space to test breath and balance before committing upward. Water cups and covered seats interrupt the climb at sensible intervals, worn smooth by people who chose readiness over pride.', room_type='path', indoor=False)
    hl_ridge_report_10 = area.room('hl_ridge_report_10', name='Return Report', desc='Arriving crews mark wind, visibility, footing, and any stranger encountered before collecting their gear deposits. A courier’s latest note places ice above the eastern turn several hours earlier than yesterday.', room_type='path', indoor=False)
    hl_high_cable_11 = area.room('hl_high_cable_11', name='Counterweight Line', desc='Stone counterweights travel inside a guarded shaft beside the court. Their guide marks expose even a slight mismatch in the lift balance, and today’s chalk shows a clean descent after morning adjustment.', room_type='path', indoor=False)
    hl_weather_pause_12 = area.room('hl_weather_pause_12', name='Storm Shelter', desc='A low shelter holds blankets, hot stones, and hooks for every pack on a delayed manifest. Its walls carry no promises about departure, only the current forecast and enough provisions to wait without becoming desperate.', room_type='path', indoor=False)
    hl_lift_chant_13 = area.room('hl_lift_chant_13', name='Lowering Call', desc='The lowering call uses shorter phrases that can be heard through wind rising from below. A carved response key lets unfamiliar hands confirm stop, slower, clear, and emergency without guessing at Kelda’s crew signals.', room_type='path', indoor=False)
    hl_thin_air_token_14 = area.room('hl_thin_air_token_14', name='Summit Token', desc='Returned breath tokens are washed and rehung beside a slate of completed crossings. A cracked token remains in a small frame, honoring the traveler who turned back early enough to guide two others down.', room_type='path', indoor=False)
