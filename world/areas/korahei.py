...
def build():
    ...
    bm_beach_market_mouth = area.room('bm_beach_market_mouth', name='Beach Market Mouth', desc='Fish smoke, split fruit, hot root, wet rope, and five overlapping arguments greet the road where stalls curve around shared hearths. A public board names today’s prices, shortages, repairs, boat arrivals, and bowls still needed, giving every smell a person or task to follow.', room_type='clearing')
    bm_saltfruit_awning = area.room('bm_saltfruit_awning', name='Saltfruit Awning', desc='Auntie Sola sorts saltfruit by grove, ripeness, storm damage, and price beneath a striped awning. Sound fruit occupies the gathering baskets while bruised pieces become preserves or shared cooking stock, and seed fruit remains marked for the growers who will replace the harvest.', room_type='clearing')
    bm_silverjack_stall = area.room('bm_silverjack_stall', name='Silverjack Stall', desc='Reef silverjack lie over clean ice with boat, fisher, catch reach, and landing hour written above each row. The first cuts go to shared hearths and reef crews, while bones, trim, and unsold fish already have broth, bait, smoke, or compost marks instead of becoming hidden waste.', room_type='clearing')
    bm_first_bite_hearth = area.room('bm_first_bite_hearth', name='First-Bite Hearth', desc='Hearth Mother Pelani portions each finished dish around a circle and names the hands, water, boat, garden, or trade that brought it there. The best first pieces pass sideways before the cook serves themself, making prestige flow toward relationship and labor rather than whoever can pay fastest.', room_type='clearing')
    bm_open_smoke_pit = area.room('bm_open_smoke_pit', name='Open Smoke Pit', desc='Adjustable racks hold fish, saltfruit, root, and shellbeans over separate levels of fragrant smoke. Fuel source, brine, start time, keeper, and intended store are chalked at the rim, letting onlookers learn preservation without wandering into food-safety work unasked.', room_type='clearing')
    bm_shellbean_table = area.room('bm_shellbean_table', name='Shellbean Table', desc='Shellbeans fill shallow baskets marked for fresh cooking, drying, seed, and storm reserve. Prices sit beside exchange and shared-hearth portions, so a tight purse changes the form of contribution without making hunger an invitation to bargain away dignity.', room_type='clearing')
    bm_net_mender_circle = area.room('bm_net_mender_circle', name='Net-Mender Circle', desc='Damaged nets circle a low frame with the tear, boat, catch, and repairer named on bright tags. Naturally shed reefcrawler shell is sorted into approved guards and teaching scraps nearby; uncertain fragments remain with the reef keeper rather than entering craft through a convenient story.', room_type='clearing')
    bm_reefstone_weighing_mat = area.room('bm_reefstone_weighing_mat', name='Reefstone Weighing Mat', desc='Reefstone offcuts are weighed against public measures and grouped by source, salt exposure, fracture, and safe use. Buyers can compare scale marks and failure samples before choosing, while intact coastal structure never becomes market stock merely because it is within reach.', room_type='clearing')
    bm_bright_paddle_rack = area.room('bm_bright_paddle_rack', name='Bright Paddle Rack', desc='Painted paddles stand by length, balance, boat, and current repair state rather than by ornament alone. Borrow and return cords record each trip, and a stripped-back sample shows how cheerful paint can conceal grain damage that deserves retirement.', room_type='clearing')
    bm_hearthroot_basket = area.room('bm_hearthroot_basket', name='Hearthroot Basket', desc='Warm-scented hearthroot fills baskets labeled by garden bed, division date, cook use, and replanting share. Mature pieces occupy the gathering row while crowns with strong buds return to the growers, tying every comforting meal to next season’s bed.', room_type='clearing')
    bm_tide_mint_mortar = area.room('bm_tide_mint_mortar', name='Tide-Mint Mortar', desc='A heavy mortar is shared among cooks blending cooling mint with salt, fruit peel, or dried root. Recipe slates record substitutions and reactions without declaring one household’s version correct, and the mortar wash schedule matters as much as the loudest flavor opinion.', room_type='clearing')
    bm_shared_bowl_bench = area.room('bm_shared_bowl_bench', name='Shared Bowl Bench', desc='Makoa guards a row of bowls with the grave attention of a child trusted with real work. Colored pegs show who has eaten, who asked for later, who needs a different ingredient, and who is carrying portions outward, turning the custom into responsive care rather than forced togetherness.', room_type='clearing')
    bm_laughing_cookfire = area.room('bm_laughing_cookfire', name='Laughing Cookfire', desc='Cooks trade jokes, corrections, and sharp disagreement around a fire wide enough for several pots. A pause spoon can stop teasing or conversation without stopping the meal, leaving room for people who want the shared warmth without becoming entertainment.', room_type='clearing')
    bm_visitor_portion_line = area.room('bm_visitor_portion_line', name='Guest Portion Circle', desc='Visitors receive food in a loose circle where ingredients, spice, price, gift, and expected return of the bowl are stated plainly. No special “best” dish is reserved for outsiders; guests share what the neighboring seats share and learn who still needs serving before asking for more.', room_type='clearing')
    bm_tool_menders_shade = area.room('bm_tool_menders_shade', name="Tool Menders' Shade", desc='Koa displays fishing, gathering, repair, and travel tools beside worn examples of what each can do badly. Sale tags include upkeep and safe-use notes, while the repair board names parts wanted and owners waiting, making trade the beginning of a tool’s relationship rather than its end.', room_type='clearing')
    bm_fish_salt_barrel = area.room('bm_fish_salt_barrel', name='Fish-Salt Barrels', desc='Barrels carry fish, salt source, pack date, brine changes, opener, and households or crews expecting shares. A tasting bowl is drawn from the working batch instead of a polished sample, and any doubtful barrel remains marked for inspection rather than discounted into someone else’s risk.', room_type='clearing')
    bm_kelp_press_stand = area.room('bm_kelp_press_stand', name='Kelp Press Stand', desc='Rinsed kelp passes through a screw press into food sheets, garden liquid, cord fiber, and compost. Beach reach and harvest date follow every bundle, with recovery plots closed on the board until reef watchers confirm that cutting can resume.', room_type='clearing')
    bm_market_story_post = area.room('bm_market_story_post', name='Market Story Post', desc='Route news, boat arrivals, craft failures, debts repaid, and invitations are tied around a post by source and date. Personal stories carry repeat, correct, or keep-here knots, giving useful rumor a visible chain while denying gossip the disguise of communal memory.', room_type='clearing')
    bm_long_spoon_hearth = area.room('bm_long_spoon_hearth', name='Long-Spoon Hearth', desc='Extra-long spoons let children, elders, seated cooks, and several arms tend one broad pot without crowding the flame. Stirring turns share authority with tasting and cleaning turns, and the recipe ledger credits the person whose correction saved a batch as carefully as its originator.', room_type='clearing')
    bm_evening_ember_ring = area.room('bm_evening_ember_ring', name='Evening Ember Ring', desc='At dusk, market crews gather unsold food, open repairs, borrowed bowls, route news, and tomorrow’s shortages around banked coals. Useful stock moves to night watches and morning hearths, names move to the next handoff, and only then do stories and first bites close the market day.', room_type='clearing')
