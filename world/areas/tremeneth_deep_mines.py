...
def build():
    ...
    bs_blackwater_sump_06 = area.room('bs_blackwater_sump_06', name='Gauge Basin', desc='Water marks and dated boards track the sump’s rise after gallery runoff and its fall during dry shifts. Doma’s food count sits beside the route closure line, making abundance and access two readings of the same water.', room_type='cave', indoor=True)
    ...
    si_sealed_imperial_cut = area.room('si_sealed_imperial_cut', name='Sealed Imperial Cut', desc='Modern braces and witness seals close an Imperial excavation whose tool scars continue beyond sight. An old haul construct still moves near the barrier, making containment an active safety boundary rather than an invitation to find the right key.', room_type='underground', indoor=True)
    si_red_wax_mark = area.room('si_red_wax_mark', name='Tampered Wax', desc='Red wax crosses the gate bolts with Warden, Ironblood, and crew impressions. One strand has been warmed and reset beside fresh claim-jumper boot marks, preserving the difference between authorized inspection and trespass.', room_type='underground', indoor=True)
    si_old_order_03 = area.room('si_old_order_03', name='Deepen Order', desc='An Imperial order demands that the cut continue until a quota is met, with brace timber and lamp oil listed as delays. Sava’s matching notes connect its seal to missing wages and injuries elsewhere in the mine.', room_type='underground', indoor=True)
    si_blocked_shaft_04 = area.room('si_blocked_shaft_04', name='Settled Shaft', desc='Stone fill rises behind a timber grille where the old shaft collapsed inward. Air ribbons still move through small gaps, while numbered blocks and dust trays reveal whether the mass is shifting without disturbing it.', room_type='underground', indoor=True)
    si_quiet_pick_05 = area.room('si_quiet_pick_05', name='Silenced Pick', desc='A wrapped pick rests where quiet inspection once tested the sealed wall. Minevermin have gnawed its handle and nested beneath the rack, leaving useful hide but no permission to swing the surviving tool.', room_type='underground', indoor=True)
    si_sealed_cut_06 = area.room('si_sealed_cut_06', name='Witness Window', desc='A thick observation grille exposes one intact wall of the cut without opening it. Survey scales, air readings, and construct sightings accumulate outside, allowing the mine to learn while the closure remains meaningful.', room_type='underground', indoor=True)
    si_red_wax_07 = area.room('si_red_wax_07', name='Layered Witness', desc='Successive wax colors record inspections after tremors, trespass, and brace replacement. Each layer carries the scope of what was opened and closed again, making access narrow, temporary, and reviewable.', room_type='underground', indoor=True)
    si_old_order_08 = area.room('si_old_order_08', name='Confiscation Order', desc='A stamped notice claims tools, carts, and extracted stone for Imperial use while charging their loss to the crew. Current annotations tie surviving item numbers to family and claim records instead of accepting the seizure as ownership.', room_type='underground', indoor=True)
    si_blocked_shaft_09 = area.room('si_blocked_shaft_09', name='Haul Scar Blockage', desc='Deep parallel grooves disappear beneath a deliberate stone barricade. Chips are labeled by repair date and source, showing where Tremen reinforced the closure without trying to excavate what the old sleds pursued.', room_type='underground', indoor=True)
    si_quiet_pick_10 = area.room('si_quiet_pick_10', name='Sounding Rack', desc='Felt-headed picks and listening cups support non-destructive checks along the outer wall. Test intervals lengthen whenever vermin, water, or nearby carts contaminate the sound, privileging a good record over a dramatic result.', room_type='underground', indoor=True)
    si_sealed_cut_11 = area.room('si_sealed_cut_11', name='Repair Seal', desc='New mortar closes a crack beside the original gate while leaving both materials distinguishable. Witness marks cross only the repair, so future crews can test its integrity without claiming to validate the entire Imperial closure.', room_type='underground', indoor=True)
    si_red_wax_12 = area.room('si_red_wax_12', name='Review Wax', desc='A red wax tablet lists the closure’s current reasons: unstable fill, active construct, disputed claims, and incomplete records. Each reason has its own reviewer and no single cleared box can open the cut.', room_type='underground', indoor=True)
    si_old_order_13 = area.room('si_old_order_13', name='Work-Through Order', desc='This brittle order commands crews to continue through a brace warning and withholds pay for stopping. Rill’s modern closure language is posted beside it, turning a historical cruelty into a specific rule the living mine rejects.', room_type='underground', indoor=True)
    si_blocked_shaft_14 = area.room('si_blocked_shaft_14', name='Seamward Bypass', desc='The Imperial shaft remains blocked behind stone and witness cord, but a maintained bypass turns toward an exposed resonance seam. Separate signs make clear that reaching adjacent evidence neither opens the cut nor resolves why both routes converge.', room_type='underground', indoor=True)
    rs_resonance_seam = area.room('rs_resonance_seam', name='Resonance Seam', desc='A mineral seam crosses the bypass beneath listening pins, sample permits, and Underhall Mapper Vaun’s competing route sketches. A displaced stone frame turns toward motion nearby while marked pockets yield carefully provenanced resonance shards.', room_type='underground', indoor=True)
    rs_listening_pin = area.room('rs_listening_pin', name='Listening Pin', desc='A bronze pin carries vibration from the seam to a removable sounding cup. Loose orelings scrape nearby stone, so each reading includes a watch for visible movement before it enters Vaun’s map.', room_type='underground', indoor=True)
    rs_hummed_dust_03 = area.room('rs_hummed_dust_03', name='Hummed Dust', desc='Fine dust forms shallow ridges around the seam while a bell-like echo crosses the gallery. Control trays on ordinary stone receive the same air and foot traffic, helping observers identify which patterns are not unique.', room_type='underground', indoor=True)
    rs_careful_sample_04 = area.room('rs_careful_sample_04', name='Source Pocket', desc='A small sampling pocket is ringed by fracture arrows, brace notes, and a catch cloth. Every removed shard retains its orientation and neighboring material, allowing forge use without severing the sample from its place.', room_type='underground', indoor=True)
    rs_unsteady_note_05 = area.room('rs_unsteady_note_05', name='Unsteady Note', desc='The listening cup’s tone drifts between repeated tests even after tool pressure is matched. Temperature, cart movement, orelings, and listener position remain listed beside the unexplained remainder.', room_type='underground', indoor=True)
    rs_resonance_seam_06 = area.room('rs_resonance_seam_06', name='Turning Seam', desc='The visible mineral line changes direction beneath a band of older fitted stone. Vaun’s maps extend it as several dotted possibilities, none promoted to a corridor, maker, or purpose the evidence cannot support.', room_type='underground', indoor=True)
    rs_listening_pin_07 = area.room('rs_listening_pin_07', name='Comparison Pins', desc='Three pins of different length enter modern mortar beside the old seam rather than piercing it. Their readings establish how the surrounding repair carries sound, protecting the mystery from an instrument-created answer.', room_type='underground', indoor=True)
    rs_hummed_dust_08 = area.room('rs_hummed_dust_08', name='Repeat Figure', desc='A dust figure resembles one copied from the Underhalls, but scale and orientation disagree. Both tracings remain side by side with the mismatch emphasized, making resemblance the beginning of a question rather than proof.', room_type='underground', indoor=True)
    rs_careful_sample_09 = area.room('rs_careful_sample_09', name='Resting Cut', desc='A sampled section has been placed under a long pause while witness marks track microfractures around it. The remaining material is left in place even though its color is richer than the removed shard.', room_type='underground', indoor=True)
    rs_unsteady_note_10 = area.room('rs_unsteady_note_10', name='Shift Disagreement', desc='Two crews recorded different tones from the same pin during adjacent shifts. Vaun has preserved lamp heat, load traffic, and instrument condition from both, giving later listeners a reason to return instead of choosing a favored result.', room_type='underground', indoor=True)
    rs_resonance_seam_11 = area.room('rs_resonance_seam_11', name='Underhall Bearing', desc='Survey bearings place the seam’s broad direction toward older Underhall stone. A red boundary note states that direction is evidence for comparison, not authority to cut a connecting route.', room_type='underground', indoor=True)
    rs_listening_pin_12 = area.room('rs_listening_pin_12', name='Calibration Pin', desc='A pin mounted in a detached block provides a known response before and after each seam reading. Chips and cup replacements are logged openly, preventing a changed instrument from masquerading as changed stone.', room_type='underground', indoor=True)
    rs_hummed_dust_13 = area.room('rs_hummed_dust_13', name='Tracing Table', desc='Soot tracings, dust figures, and route bearings overlap on a table without touching the seam itself. Blank margins identify where the line leaves observed stone, preserving the point at which the map becomes inference.', room_type='underground', indoor=True)
    rs_careful_sample_14 = area.room('rs_careful_sample_14', name='Redrawn Seam', desc='Vaun’s final working copy redraws the seam in red and bundles a provenance-rich tracing for Nelli. The return ledger records what may travel to the Underhalls—evidence and questions—while stone and permission remain here.', room_type='underground', indoor=True)
