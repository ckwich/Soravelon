...
Keywords: area builder, zone, room, area, build, grid, world position, auto layout, flight point, flight route, cross-zone exit, spawn, named mob, patrol, trigger, quest, material, gathering pool, lore fragment, validate zone, area validator, item definition, spawn point, greeter room, respawn point, reconcile, orphan, evict
...
- `tests/test_area_builder.py` — Comprehensive test suite with `AreaBuilderTestBase`, including `TestReconciliation`
...
- **Reconciliation on rebuild:** `build()` calls `_reconcile_stale_objects()` which hard-deletes zone-owned DB objects not in the current spec. Order: exits → NPCs → mobs → rooms (rooms last so contents are cleaned first). Players in deleted rooms are evicted to `respawn_point` → `greeter_room` → `DEFAULT_HOME` fallback chain. Runtime-spawned mobs (tagged with `MOB_INSTANCE_TAG_CATEGORY`) are exempt — only builder-authored `mob()` objects are reconciled.
- **Build report:** `build()` returns a dict with `zone_id`, `rooms_created`, `exits_created`, `warnings`, `unresolved_exits`, and `reconciled` (counts of `rooms_deleted`, `exits_deleted`, `npcs_deleted`, `mobs_deleted`, `players_evicted`).
...
- **`mob()` creates DB objects with zone tags:** `mob()` creates actual mob objects (unlike `spawn()` which stores defs). Sets both `mob_obj.db.zone_id` and `mob_obj.tags.add(zone_id, category="zone_id")` for reconciliation tag-based lookups.
...
- **Trigger events:** `trigger()` accepts events: `on_enter`, `on_exit`, `on_first_visit`, `on_mob_death`, `on_examine`. Stored on `room.db.triggers` list.
...
4. **`build()` order matters** — cross-zone exits → node init → patrols → auto_layout → reconciliation → zone registry → list attr init → gathering pool init
...
11. **Reconciliation preserves runtime mobs** — `_reconcile_stale_objects()` skips mobs tagged with `MOB_INSTANCE_TAG_CATEGORY` (runtime-spawned). Only builder `mob()` objects are deleted when removed from spec
12. **`mob()` must tag with `zone_id` category** — both `db.zone_id` and `tags.add(zone_id, category="zone_id")` are required for reconciliation tag-based lookup to work
...
- **Mob Spawner:** `world/mob_spawner.py` — `MOB_INSTANCE_TAG_CATEGORY` used by reconciliation to identify runtime mobs
