...
- `world/skill_engine.py` — `get_skill_value()` for appraisal check, `accumulate_skill_use()` for passive gain
...
3. **Skill accumulation on success** — `accumulate_skill_use(character, "appraisal")` called only on successful inspect. Uses the session-accumulator pattern (ndb → DB batch), not the older `record_skill_use`
