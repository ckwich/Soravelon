...
| Builder save, packaging, and UI rebuild | **active at B1.2** | Builder commits `47ceb98` and `e1987ad` establish the Rust-owned typed transaction and native manual-save client. Commit `445a517` puts browser-dev manual Save/Save As behind that same `saveZoneFile(path, zone)` API and a single typed `/save-zone-file` endpoint. The development server now owns serialization, hard-error validation, and atomic replacement; a real temporary-filesystem regression proves validation failure preserves both the original and an existing trusted backup without leaving a temp write. All 34 frontend/development tests, the production build, 42 passing sidecar tests with 15 platform skips, and both Rust transaction tests pass. Auto-save and World Map batch export still use the generic serialized-content writer. Rust tests still require a test-only empty `externalBin` override because the target sidecar is absent; that remains B1.3 evidence. | Cut auto-save and batch export to typed transactions, then remove the generic zone writer and prove write/replacement recovery failures. |
...
**Exact next slice:** add RED caller contracts proving auto-save and World Map
batch export pass typed `ZoneModel` values to `saveZoneFile` and never call the
generic serialized-content writer. Make them GREEN, then remove `writeZoneFile`,
the Rust `write_zone_file` command, and the browser `/write-zone-file` endpoint.
Do not remove the structured preflight that feeds accessible Problems UI. Leave
write/replacement fault injection and recovery hardening for the following
slice. The user explicitly removed the Builder
