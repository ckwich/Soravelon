...
| Revisioned world content | **M2.1 complete; active at M2.2** | Commits `a24e231`, `a79957d`, `e42728e`, `4d6d855`, and `fb27f1d` establish pure literal compilation, immutable semantic manifests, topology/Social/registry/quest/action validation, and zero direct runtime-mutation escape hatches. Every mutable operation in the live 20-zone manifest now has a stable semantic change identity. Pure planning fails closed on unmodelled or duplicate identities, names creates/updates/moves/deletes, ignores source-only formatting changes, and classifies destructive/player-impacting changes including occupied-room, navigation, active-quest, inventory, relationship-memory, and travel risks. Runtime `player_stock` is preserved. Startup still mutates AreaBuilder-owned state; runtime revision records, occupancy checks, locking, atomic apply/rollback, and idempotent restart proof remain. | Build M2.2 read-only `validate`/`plan` against durable applied-manifest state before implementing apply. |
