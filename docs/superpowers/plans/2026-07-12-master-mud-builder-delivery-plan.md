...
| Revisioned world content | **active at M2.1** | Commit `a24e231` adds a pure AST source-authority compiler with immutable operation/source-location records and fail-closed unknown-operation diagnostics. Commit `a79957d` adds literal `room_role`, `initial_room_state`, `vendor`, and `medic` operations and removes all 137 direct `.db`/`.tags` mutations from the 21 live Python area sources; the compiler audit is now zero violations. Runtime `player_stock` is deliberately preserved rather than reset by authored vendor configuration. Startup still mutates AreaBuilder-owned runtime state; no full immutable definition graph, revision plan/apply record, rollback, or idempotent restart proof exists. | Extend the compiler from operation inventory to immutable literal argument/reference definitions, reject dynamic/unmodelled expressions, then build mutation-free world manifests. |
