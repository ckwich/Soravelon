...
| Builder save, packaging, and UI rebuild | B2 **automated acceptance complete; observed usability deferred**; B1.3 native package certification is externally blocked; B1.2 **complete** | Builder commits through `449f876` establish typed offline persistence, the target-aware packaging spine, the offline-workbench rebuild, compact accessibility/reflow protection, explicit area-open/authored-save/recovery state boundaries, and a repeatable semantic accessibility matrix over the highest-risk document states. At exact `449f876`, the live in-app browser matrix covers all five work modes plus Home, World, and Templates; keyboard shortcuts; long labels; offline/read-only truth; and wide, 200%-equivalent, and 400%-equivalent reflow without horizontal document overflow. The exact clean worktree passes the production build and all 161 frontend tests; both Node dependency audits report zero vulnerabilities. Cole explicitly deferred the observed usability session, so that human acceptance is not claimed. Commit `de5595d` adds the read-only four-runner native package matrix. The repository now has an authorized Git remote, but hosted run `29305692460` was rejected before checkout on all four runners by the GitHub account payment/spending-limit gate. Exact clean-clone macOS ARM gates remain green; no other native target is claimed certified. This packaging gate is independent of Lightsail hosting and live-game uptime. | Perform the deferred observed usability session when Cole is available. Resolve the separate GitHub account gate and run the four native jobs before closing B1.3. |
...
| Production hosting correction (explicit user override of the earlier gate order) | **single-instance production infrastructure live and stable; final release evidence and human acceptance still open** | Cole explicitly authorized production deployment and one existing USD 12/month Lightsail instance, while prohibiting additional instances or purchases without authorization. The only instance is Ubuntu 24.04 x86_64 in Oregon with a static IP, PostgreSQL 16, Python 3.12, Nginx, Certbot, UFW, unattended upgrades, and a 2 GB swap file. Soravelon is deployed from clean exact SHA `3f928e100a15cd865fc0eda45f5cb1248c090d44`; `systemd` is active with public Telnet `4000` plus loopback web `4001`, WebSocket `4002`, and AMP `4005`. Cloudflare routes apex and `www` to the static IP, `ws` to the WebSocket proxy, and DNS-only `telnet` to the instance; Full (strict) TLS is enabled and the origin certificate covers apex, `www`, and `ws`. Content revision 1 is current across all 20 zones at manifest `b1f1ceb5813c38c9a3071b56e0f23d6dbde224aa0d11a1f439f63fd561598e0b`. Public admin authentication, Telnet, HTTPS stock webclient/OOB, and WSS smokes pass; the clean exact-SHA canonical suite passes 2,189 tests with 5 skips. The bootstrap backup `/var/backups/soravelon/soravelon-bootstrap-2d39d7d-20260714.dump` is 3,432,109 bytes with SHA-256 `04313f3e9e94f9447119e80290ac7e6e75a4b3bb25a555b7ffd21453b5488920`. A later read-only check again proved the clean exact SHA, active service, intended listeners, apex/`www` HTTP 200, and no recent traceback, critical, or failed service log entry. At cutover time the retained candidate still belonged to `27cbb17`; the next row records the later exact-production-SHA reconciliation. The Builder matrix remains blocked as described above, and Cole deferred the observed 60–90 minute/co-op/living-world/accessibility playtest. | Do not create another instance or paid resource. Resolve the GitHub billing gate and retain the exact-SHA Builder matrix; perform the deferred observed acceptance; reconcile final evidence without fabricating cross-SHA proof. Do not start C1. |
| Production-SHA M6 reconciliation (supersedes the stale cross-SHA clause above) | **automated MUD and host evidence complete at production SHA; Builder distribution and observed acceptance pending** | The runtime delta from retained evidence SHA `27cbb17` to deployed SHA `3f928e100a15cd865fc0eda45f5cb1248c090d44` is limited to production configuration, Nginx/deployment documentation, and focused tests. Thirteen production-settings/startup tests pass, Bandit 1.8.6 reports zero findings, and `pip-audit` reports only the already governed Twisted 24.11.0 advisory whose fixed release is outside Evennia 6.1's supported range. Two newly independent PostgreSQL rehearsals, `soravelon_rehearsal_prod1_*` and `soravelon_rehearsal_prod2_*`, each prove fresh `uninitialized -> initialized -> no-op -> current`, independently restored `current -> no-op -> no-op`, revision 1, exact deployed Git identity, and manifest `b1f1ceb5813c38c9a3071b56e0f23d6dbde224aa0d11a1f439f63fd561598e0b`. Both lifecycle receipts prove cold start, Portal-preserving reload, zero-listener stop, second start, enabled-VM reboot recovery, explicit whole-cgroup SIGKILL restart with an incremented restart counter, all three intended player listeners, and zero authored-content writes. Both 16-step candidates pass rollback injection, all nine gameplay verticals, all 2,189 canonical tests in 1,521.632 and 1,520.388 seconds, plus real Telnet, forwarded-HTTPS webclient, and RFC6455 WebSocket round trips. Backups hash to `716c4b695b697564c929185dd8db4ed4b659693e915f93f03ea3b2d93995faf9` and `5c3a3ba34aa47407341955399289e3bc8d8c0846a55bc377358c266fdee24af9`; lifecycle receipts hash to `5813480ba6b6cae8bbf414df73557f593df4778d511fdea412988d067bd71f21` and `80f91709ad58c77da4dda5004abcb10de55b308684ab62fe34d6e91d00eec0cc`; candidate logs hash to `e4cd014a1538236b74d36e891426c194aeac1bbb825f10ecde17b7d248a73512` and `5633b25a62b6a76969462f37f1bb0bb8116a7686fe4501144c271092180b36dd`. The secret-free bundle at `/Users/ckwichman/Documents/Projects/soravelon-release-evidence-3f928e1` validates all automated artifacts and fails exactly 25 external/human fields: four exact-Builder-SHA native packages and the deferred observed acceptance receipt. A final read-only public check returns HTTP 200 from apex and `www`, completes the Telnet login-banner/`look` round trip, and negotiates RFC6455 over TLS with `v1.evennia.com` at `ws.soravelon.com`. GitHub Actions is not an uptime or deployment dependency. | Keep the live instance stable and do not create another paid resource. When Cole is available, perform the observed acceptance. Separately resolve GitHub billing and run the four native Builder jobs; then fill only those retained fields and rerun the final record validator. Do not start C1. |
...
and locally green, and the Builder now has an authorized Git remote. The exact
hosted matrix remains blocked before checkout by GitHub's account payment or
spending-limit gate; this is independent of Lightsail hosting and live-game
uptime. B1.3 closes only when all four exact-SHA results and retained packages
are green. The user explicitly removed the Builder GSD-workflow requirement for
this program; continue with the repository's tests and scoped commit discipline.
...
### B2 execution status — 2026-07-14

**Status:** automated acceptance complete; observed usability deferred by Cole.
Builder commits `3f5fcba`, `b4209b3`, `9fff200`,
`ac46bdf`, `258b215`, `343715f`, `09afc3f`, `8785ce1`, `bce55a8`,
`af60e2c`, `2f6f839`, `b5aaa41`, `558b0d9`, `92c13fb`, `48efe06`,
`287dafb`, `2aeca28`, `a44f744`, `4a40d02`, `895b72a`, `dae37fc`,
`72fbf0d`, `417b67d`, `4f7d654`, `d07a0a4`, `26eb5b2`, `67e520e`,
`fb76c88`, `c3d9f1e`, `d3ddc32`, `845617e`, `f2f657d`, and `449f876`
...
At exact `dae37fc`, `workspaceSessionStore` owns the active workspace route and
focused-dialogue NPC index outside the temporal area content store. Its API
requires live area availability for area-only routes, fails closed to Home
without a document, clears focused dialogue on non-dialogue navigation, and
keeps area open/close lifecycle transitions explicit. App, World Overview,
the Inspector, and NPC dialogue entry points consume the dedicated authority;
`editorStore` no longer exposes or mirrors workspace route state.

The first compact live gate exposed a real coordinator race: `loadZone()`
opened Map through the new lifecycle authority, then `handleZoneLoaded()`
overwrote it with Home using the pre-load render's stale no-area closure. The
duplicate coordinator write is removed, so area load/create lifecycle is the
sole load-time route authority. In a fresh disposable Soravelon clone at
900x700, `Ctrl+1` with no area stayed on Home; opening Vael's Crossing entered
the 108-room Map; `Ctrl+4` entered Narrative; selecting `npc_greeter_maren`
and its real Edit Dialogue action opened the focused Dialogue editor; closing
it returned to Narrative; `Ctrl+0` retained the local area on Home; and
`Ctrl+1` resumed Map. All browser resources stayed on `127.0.0.1`, the authored
checkout remained clean, there was no document-level horizontal overflow, and
the browser reported zero warnings or errors. Exact staged diff SHA
`485cea53899cc41e9b311030225b6daaffdaaabb27a449578df42e15a5ba378c`
matched the clean clone byte-for-byte and passed the production build, all 115
frontend tests, 69 sidecar tests with 16 platform skips, all 17 Rust tests,
Rust format, packaged ARM sidecar smoke, and both Node dependency audit scopes
with zero vulnerabilities.

At exact `72fbf0d`, `areaDocumentStore` owns authored-area file identity,
dirty state, and the last safely saved structured snapshot outside temporal
area content. Opening, recovery, new-document, close, mutation, save, undo, and
redo flows all route through that authority; `editorStore` no longer exposes
or mirrors document lifecycle state. Starting a new area now explicitly clears
the previous document's saved snapshot, and undo/redo recompute dirty truth
against the saved structured snapshot rather than merely replaying a stale
boolean.

In a fresh disposable Soravelon clone at 1440x900, editing the real
`hg_arrival` room made the authored document dirty; `Ctrl+S` changed only the
disposable source, created an exact backup of the original, and returned the
document to clean. `Ctrl+Z` restored the original room value and correctly
returned the document to dirty; `Ctrl+Y` restored the saved value and clean
state. No temp write candidate remained, all browser resources stayed on
`127.0.0.1`, there was no document-level horizontal overflow, and the browser
reported zero warnings or errors. Exact staged diff SHA
`2e936290c3f87775bf3d2ae674bdafcb36ea29ff1a093a6318f9dd301c80bf97`
matched the clean clone byte-for-byte and passed the production build, all 118
frontend tests, 69 sidecar tests with 16 platform skips, all 17 Rust tests,
Rust format, packaged ARM sidecar smoke, and both Node dependency audit scopes
with zero vulnerabilities.

At exact `417b67d`, `areaLayoutStore` owns editor-only room positions,
persisted-layout load state, active zone identity, and room-position lifecycle
outside temporal authored content. Area open, new-document, close, create,
rename, delete, map projection, drag, and save consumers all route through the
dedicated boundary; `editorStore` no longer exposes or mirrors layout state.
Persisted-layout reads now block the initial BFS seed until their outcome is
known, reject late results from a previously opened zone, filter stale room
identifiers, and preserve room identity or user-position changes made while a
read is still in flight.

In a fresh disposable Soravelon clone with isolated Builder app data, the live
workbench restored deliberately authored positions for `hg_arrival` and
`hg_gatehouse` exactly, ignored a stale layout-only room, and issued no
save-layout request while opening. The persisted layout and authored area
remained byte-identical. Both 1440x900 and 900x700 passes used only local
`127.0.0.1` resources, had no document-level horizontal overflow, and reported
zero browser warnings or errors. Exact staged diff SHA
`7ad29ccbbdf5a05f22995351b0707cdeb81022d9001d1d48faea51a415cd42d8`
matched the clean clone byte-for-byte and passed the production build, all 122
frontend tests, 69 sidecar tests with 16 platform skips, all 17 Rust tests,
Rust format, packaged ARM sidecar smoke, and both Node dependency audit scopes
with zero vulnerabilities.

At exact `4f7d654`, the remaining temporal `ZoneModel` and mutation authority
is explicitly named `areaContentStore`; every workbench, panel, service, test,
and undo/redo consumer uses that interface directly. The stale generic
`editorStore` module and symbol are removed rather than retained as a
compatibility facade. Together with the dedicated document, layout, selection,
history, validation/review, session, context, and library stores above, this
completes the planned store-ownership split while leaving parser, serializer,
validator, and IPC services unchanged.

The live regression exposed a pre-existing no-op mutation defect: after undo
returned a room to its saved value, moving to Content blurred another unchanged
room field, and `updateRoom` marked the document dirty anyway. A focused red
test now proves unchanged field commits neither dirty the authored document nor
add visible history. In a fresh disposable Soravelon clone at 900x700 and
1440x900, the real 108-room Map accepted a room edit, `Ctrl+Z` restored the
saved value and clean state, entering Content kept it clean, searching for
`npc_greeter_maren` selected the real NPC Inspector, and recovery cleanup left
the authored source byte-identical. All resources stayed on `127.0.0.1`, there
was no document-level horizontal overflow, and the browser reported zero
warnings or errors. Exact staged diff SHA
`270d59b23bb884f9692e893c4091d09c961a1ec04b6269252c2a0b818cd07efc`
matched the clean clone byte-for-byte and passed the production build, all 123
frontend tests, and both Node dependency audit scopes with zero
vulnerabilities. Sidecar and Rust sources are unchanged from `417b67d`.

At exact `d07a0a4`, `AreaWorkbenchCoordinator` owns the single route boundary
for Map, Content, Narrative, Social, Review, bulk Descriptions, and focused
Dialogue. `App.tsx` no longer imports or renders those seven workbench
implementations directly; it retains shell, document/save/recovery, local-tool,
and cross-area coordination. The new boundary uses the existing area content,
workspace session, and selection authorities rather than introducing route or
model state.

In a fresh disposable Soravelon clone at 900x700 and 1440x900, one live pass
opened the real 108-room and 221-exit Map, then routed through Content, Review,
Narrative, Social, all 108 bulk room descriptions, and
`npc_greeter_maren`'s real focused Dialogue editor. The authored source and
recovery paths stayed byte-identical and clean. Every browser resource stayed
on `127.0.0.1`, neither width had document-level horizontal overflow, and the
browser reported zero warnings or errors. Exact staged diff SHA
`6d2166bd3078f7e65a06d99bae67ecade9ec26ce6957decf8d6616e1143cb641`
matched the clean clone byte-for-byte and passed the production build, all 124
frontend tests, and both Node dependency audit scopes with zero
vulnerabilities. Sidecar and Rust sources are unchanged from `417b67d`.

At exact `26eb5b2`, `LocalDocumentCoordinator` owns Home, read-only World
Overview, and Template Library routing. `App.tsx` no longer imports or renders
those local-document implementations directly; it supplies the existing file
browser, world-navigation guard, and template save boundary. Together with
`AreaWorkbenchCoordinator`, every center route now enters through an explicit
local-document or area-workbench coordinator without adding route state.

In a fresh disposable Soravelon clone at 900x700 and 1440x900, Home remained a
truthful no-document workspace; after local folder authorization, World
Overview accounted for all 20 canonical files and retained the existing 11
world-link findings; and the real 81-template reusable-creature library opened
cleanly. Vael's Crossing and `mob_templates.py` stayed byte-identical. Every
browser resource stayed on `127.0.0.1`, neither width had document-level
horizontal overflow, and the browser reported zero warnings or errors. Exact
staged diff SHA
`29f58e87da1d8f518bf7e38d0e2a7a0f5df8093f063933eff0b704079f3fca33`
matched the clean clone byte-for-byte and passed the production build, all 125
frontend tests, and both Node dependency audit scopes with zero
vulnerabilities. Sidecar and Rust sources are unchanged from `417b67d`.

At exact `67e520e`, one tested `areaDocumentPersistence` service owns the
model-validator, serializer, generated-source validator, atomic authored-file
write, and optional nonfatal layout-write sequence. Save, Save As, and World
Overview's reverse-exit workflow now consume that policy instead of duplicating
it in UI coordinators. Validation blocks before any source write; a layout
failure cannot turn a completed authored-file transaction into a false source
failure. UI-only file selection, failure presentation, save-state recording,
and navigation remain with their existing coordinators.

In a fresh disposable Soravelon clone at 900x700, editing the real
`hg_arrival` room and pressing `Ctrl+S` changed the authorized clone source,
created a backup whose SHA-256 exactly matched the original source, left no
temporary write candidate, and wrote the isolated layout document. `Ctrl+Z`
restored the original value and dirty state; `Ctrl+Y` restored the saved value
and clean state. Every browser resource stayed on `127.0.0.1`, the document
had no horizontal or vertical overflow, and the browser reported no console or
page errors. Exact staged diff SHA
`d1f5fec3863eedd799baefb28aaf4510f281509ab407277edd201cfcd6aeeb29`
matched the clean clone byte-for-byte and passed the production build, all 129
frontend tests, and both Node dependency audit scopes with zero
vulnerabilities. Sidecar and Rust sources are unchanged from `417b67d`.

At exact `fb76c88`, the desktop workbench applies an explicit responsive-rail
policy: both contextual rails collapse at widths through 1099 pixels, the
Inspector collapses at 1100--1319 pixels, and both rails remain available at
wider desktop widths. Collapsed rails are inert and hidden from assistive
technology; active workspace modes expose `aria-current`; route transitions
move focus to the workbench heading; React Flow maps and their room, zone, and
exit elements expose concise accessible names; and reduced-motion preferences
disable nonessential animation. Removing the shell's fixed 860-pixel minimum
width closes the real 720-pixel reflow failure instead of masking it with a
manual rail toggle.

In a fresh disposable Soravelon clone, the real 108-room Vael's Crossing map
passed at 1440x900, 900x700, and 720x700. The compact widths collapsed both
rails without horizontal or vertical document overflow; Content and Map
transitions focused the correct heading; the wide layout preserved both rails;
and the named map, nodes, exits, and active tab exposed the intended semantic
state. The browser reported zero errors or warnings. Exact staged diff SHA
`8203b02e4f11e0c47aa8553f160cf0038d814a55c146e44b33d5f07dbe899e21`
matched the clean clone byte-for-byte and passed the production build, all 137
frontend tests, and both Node dependency audit scopes with zero
vulnerabilities. Sidecar and Rust sources are unchanged from `417b67d`.

At exact `c3d9f1e`, local area opening has an explicit loading/failure state in
Home instead of sending a parser failure only to the hidden area-outline rail.
The state names the exact attempted path, preserves the parser/contract reason,
makes clear that the source is untouched and no editable document exists, and
offers keyboard-operable retry and dismiss actions. The failure heading receives
focus and the state uses polite loading and assertive failure semantics. Direct
and recent-file opens share the same state; a successful parse clears it before
entering the existing area workbench. Testing Library, user-event, and jsdom are
test-only dependencies for the remaining interaction matrix.

The red gate first proved that `FileBrowser` discarded the attempted path and
that no central status component existed. Exact staged diff SHA
`49aac1a366b9417fa1d8f3bbe398350d636623767f6595fcacf1bebdff6bf25b`
then matched a fresh clean clone, passed the production build, all 140 frontend
tests across 47 files, and both Node dependency audit scopes with zero
vulnerabilities. The interaction tests prove exact-path propagation, absence of
zone mutation on failure, accessible evidence, and working retry/dismiss
controls. Sidecar and Rust sources are unchanged from `417b67d`.

At exact `d3ddc32`, authored-file writes have a separate lifecycle from
recovery-copy writes. Header, compact status, and Review announce validating
and writing, a completed local atomic transaction, newer edits that arrived
during that transaction, and an assertive authored-save failure without
borrowing recovery language. Save and Save As are disabled while the authored
transaction is in flight. A document generation token prevents a late
completion from overwriting a replacement document, while the saved snapshot
and current live model are compared so an edit made during the write remains
truthfully dirty.

The red gate first proved that authored status was absent, competing writes
remained enabled, and `recordSave` erased in-flight edits or could update a
replacement document. Exact staged diff SHA
`a53c3ad513c59a8e4ae9c5e4d045852a44dbcc0acba20f6989fd10de2dc541e5`
then matched a fresh clean clone, passed the production build, all 149 frontend
tests across 48 files, and both Node dependency audit scopes with zero
vulnerabilities. The interaction gate proves disabled duplicate actions and
real progress announcement; store tests prove both in-flight identity cases.
Sidecar and Rust sources are unchanged from `417b67d`.

At exact `845617e`, the local template-library document has the same truthful
authored lifecycle without sharing the area document store. Header and compact
status distinguish validating/writing, clean local-atomic success,
saved-previous-state with newer edits, and assertive failure. The library's
existing in-flight edit protection now also uses a document generation token,
so a late completion cannot replace the source baseline of a newly opened
library. Loading a library resets stale save feedback; the template's own save
and replacement controls remain disabled during its transaction.

The red gate first proved that template presentation remained merely
clean/dirty and that `recordSave` could update a replacement library. Exact
staged diff SHA
`7cd6d71779992985d2bbe8d838bd5e3ef6e8ebd1e31731a7fc48440b31342f20`
then matched a fresh clean clone, passed the production build, all 152 frontend
tests across 48 files, and both Node dependency audit scopes with zero
vulnerabilities. Store tests prove in-flight edits and replacement identity;
presentation and header tests prove the independent saving and failure copy.
Sidecar and Rust sources are unchanged from `417b67d`.

At exact `f2f657d`, recovery is an explicit provenance-preserving choice rather
than a best-effort prompt. The modal names both the authored source and recovery
copy, puts initial focus on Recover, has no inert close affordance, disables
duplicate actions while parsing or discarding, and keeps an assertive exact
failure visible. A failed recovery parse never applies a partial model or closes
the choice; a discard failure no longer claims that the artifact disappeared;
and a changed active-document identity fails before parsing.

The red gate first proved that recover failure merely logged and closed while
discard failure was ignored. Exact staged diff SHA
`2d0c0c06448408012730de41036dd93126fe4eb2834cc878af4f09661583a01e`
then matched a fresh clean clone, passed the production build, all 157 frontend
tests across 50 files, and both Node dependency audit scopes with zero
vulnerabilities. Real DOM interaction tests prove focus, explicit choices,
busy-state lockout, paths, and alerts; service tests prove parse, discard, and
document-identity failure semantics. Sidecar and Rust sources are unchanged
from `417b67d`.

At exact `449f876`, axe-core `4.12.1` provides a repeatable semantic
accessibility gate over the Builder's empty, loading, parse-failure,
validation/read-only, authored-save progress/failure, and recovery
choice/failure surfaces. The gate disables only color contrast, which jsdom
cannot calculate, and excludes only Base UI's library-owned WebKit/VoiceOver
focus sentinels; application controls remain subject to the full command-name
and semantic rule set. Exact staged diff SHA
`b499b506a7f66a6324748bff1fce3e81f6e5071280fe49c9ae4be47edfa50795`
matched a fresh clean worktree, passed the production build, all 161 frontend
tests across 51 files, and both Node dependency audit scopes with zero
vulnerabilities. Sidecar and Rust sources are unchanged from `417b67d`.

At exact `449f876`, a fresh in-app browser run opened the real 108-room,
221-exit Vael's Crossing file and exercised Home, Map & exits, Content,
Narrative, Social, Review, the read-only World overview, and the empty offline
Template Library. `Ctrl+1` through `Ctrl+5` moved through every area work mode;
`Ctrl+6` and `Ctrl+7` opened the two local-document modes. The selected Social
connection `gathering:vc_inn_travelers -> npc:npc_innkeeper_whistle` retained
its full accessible name, visible focus, trust, endpoint, and policy-tag detail
at compact width. Sixteen inspected screenshots cover 1440x900, 720x700
(200%-zoom reflow equivalent), and 360x740 (400%-zoom reflow equivalent).
Every measured state kept document, body, and viewport width equal with no
horizontal document overflow. The 360px Map and Template states retained
navigation, document identity, save/offline state, and usable primary content.
The exact clean worktree passed the production build and all 161 frontend tests
across 51 files; both Node dependency audit scopes reported zero
vulnerabilities. The unrelated local duplicate IPC files caused only the dirty
checkout build to fail and remain preserved; they were excluded from the exact
commit gate.

This closes the automated mode, state, keyboard, long-label, screenshot, and
reflow acceptance surface for B2. It does not claim the observed usability
session that Cole explicitly deferred. The architecture and persistence
ownership split is sufficient for the accepted surface; further coordinator
decomposition must be justified by a proven contract rather than file-size
reduction alone.
...
`b5aaa41`, `558b0d9`, `92c13fb`, `48efe06`, `287dafb`, `2aeca28`, `a44f744`, `4a40d02`, `895b72a`, `dae37fc`, `72fbf0d`, `417b67d`, `4f7d654`, `d07a0a4`, or `26eb5b2`; their evidence is the exact
clean-clone gates above. No hosted run exists for `67e520e`, `fb76c88`,
`c3d9f1e`, `d3ddc32`, `845617e`, `f2f657d`, or `449f876`; their evidence is the exact
clean-clone gates above. GitHub Actions is required only for the native package
certification matrix, not for Lightsail hosting or live-game uptime.
...
green and retained at the exact production SHA
`3f928e100a15cd865fc0eda45f5cb1248c090d44`, with identical manifest
...
pass all 16 steps, all nine gameplay verticals, all 2,189 canonical tests, the
complete Linux lifecycle, and real Telnet, forwarded-HTTPS webclient, and
RFC6455 WebSocket gates. Production is live at that same SHA by Cole's explicit
override of the earlier gate order. The partial evidence record now fails only
the exact-Builder-SHA four-platform package fields and Cole's deferred observed
acceptance receipt; GitHub Actions is not required for hosting or uptime.
