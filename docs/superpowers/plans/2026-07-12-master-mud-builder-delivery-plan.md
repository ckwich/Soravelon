...
| Production configuration hardening | **complete** for source, disposable-PostgreSQL, restored-copy, and most Linux lifecycle contracts; reload remains **active** | `ceea166` and `3676aa7` established environment-owned secrets, fail-closed production settings, listener checks, and `tests/test_production_settings.py`. A 2026-07-12 disposable restored-copy rehearsal preserved an exact sentinel through `pg_dump -Fc`/`pg_restore`, then passed the full PostgreSQL preflight. Commit `34363e2` closes the real Ubuntu/systemd failures found in a clean committed checkout: missing admin `#1`, ignored log directory, and virtualenv `PATH` for Evennia's internal `twistd` launch. Start, forced whole-cgroup crash/restart, clean stop, and enabled VM reboot recovery pass with all four intended listeners. No production deployment is claimed. | Remove startup AreaBuilder mutation in M2, then repeat reload; the current multi-minute mutation makes `ExecReload` time out and leaves the Portal without the Server. |
| Executable release verification | **active** | `df09dd8` added `scripts/verify_release.py`; `b52b50f` makes every isolated CI shard migrate its own disposable base database. Commit `3076197` adds the proven Evennia attribute lookup index, bounds exact ObjectDB tag reads without global aggregation, and makes the full-world integration test exercise production-like autocommit. Sterile PostgreSQL shards 1–4 pass respectively with 523, 453, 435, and 517 tests; this includes economy, quest, and inventory concurrency plus all 20 deferred cross-zone exits. Commit `62787ea` closes six live-command help gaps and stale lookup seams exposed by shards 1 and 3. The restored-copy preflight is green. Framework-wide `makemigrations --check` proposes only Evennia-owned dynamic proxy migrations plus an upstream tag-index state rename; Soravelon's owned `world` migration state is clean, so no project migration was fabricated in `site-packages`. The master M6 twice-clean gate has not run. | Implement M2's no-startup-mutation contract, repeat the Linux reload gate, then close M1; the twice-clean full release gate remains M6 evidence. |
| Revisioned world content | **M2.1 complete; active at M2.2** | The M2.1 commit chain establishes pure literal compilation, immutable semantic manifests, complete semantic identity, fail-closed topology/registry/action validation, and destructive/player-impact classification. M2.2 now has a durable `WorldContentRevision` schema with complete lossless manifest evidence, Git/plan/lifecycle metadata, hash-integrity verification, and a database constraint permitting only one applied revision. Read-only `worldcontent validate`, `plan`, `status`, and `bootstrap-check` prove uninitialized/current/drifted/corrupt states without writes. The exact full-world bootstrap verifier reconciles zones, rooms, exits, NPCs, quests, items, registries, Social Web state, nodes, flight, and other AreaBuilder-owned runtime state to the compiled manifest; its live startup gate passes. `worldcontent bootstrap-adopt --git-commit ...` transactionally repeats that exact proof and records the initial applied manifest, empty-to-current semantic plan, and Git provenance without mutating world content; it refuses drift, invalid provenance, or pre-existing revision authority. Apply preflight resolves destructive room deletions to exact live Character occupants and refuses them without explicit maintenance approval. A deterministic materializer replays immutable operations without importing source, resolves symbolic and cross-zone topology fail-closed, exact-verifies the full world, and reconciles omitted zones with approved occupant eviction. `worldcontent apply` holds a stable singleton database row lock, verifies the applied baseline, replans under lock, materializes and exact-verifies inside a savepoint, and only then supersedes old authority. Injected failure rolls database/runtime state back, repairs process-local registries from the prior manifest, persists failed attempt evidence, and permits a later durable retry; applying the current manifest is a proven no-op. `worldcontent rollback --revision-id ...` applies a historical immutable manifest through the same locked lifecycle as a new attempt, marks the undone revision `rolled_back`, and records its historical target in the new plan. Manifest hashes are indexed rather than unique because immutable application attempts—not semantic payloads—are the durable revision identity. Runtime `player_stock` is preserved. Startup still mutates AreaBuilder-owned state; verification-only startup and idempotent restart/reload proof remain. | Replace startup rebuilding with applied-revision verification and runtime-registry hydration, then prove restart/reload idempotence and close M2.2. |
| Builder Social DSL round trip | **complete** | Builder commits through `687a589` prove fail-closed parsing, typed storage, IPC preservation, serialization, validation, explicit dynamic-helper rejection, a real local sidecar save/reopen path, and a 57-test live-area gate. `fedd997` completes typed Social node/edge creation, outline and command-palette selection, structured inspectors, reference-safe store mutations, and linked navigation without raw JSON or a MUD connection. Commit `693888a` keeps the new M2 literal runtime configuration offline and round-trippable, preserves structured room-state payloads, and removes player-specific runtime stock from the authoring contract. The live Soravelon sidecar suite passes 70 tests, frontend passes 48, and the production build passes; no Builder-to-MUD connection exists. The unrelated deleted `src-tauri/binaries/.gitkeep` remains preserved. | Re-run B1.1 after parser, validator, serializer, store, Social UI, or server grammar changes. |
