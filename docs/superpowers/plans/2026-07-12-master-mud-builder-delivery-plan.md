...
| Revisioned world content | **M2.1 and M2.2 complete; active at M2.3** | The M2.1 chain establishes pure literal compilation, immutable semantic manifests, complete identity, fail-closed topology/registry/action validation, and destructive/player-impact classification. M2.2 adds durable revision evidence, one-applied authority, exact bootstrap verification/adoption, explicit fresh initialization, locked atomic apply, occupied-room protection, durable failure recovery/retry, explicit historical rollback, omitted-zone reconciliation, and exact full-world manifest replay without source imports. Manifest hashes are indexed rather than unique because immutable application attempts—not semantic payloads—are revision identity. Startup no longer contains the legacy area loader: it requires source, applied manifest, and runtime to agree exactly, then hydrates only process-local Zone/flight registries from existing rows. Two complete starts produce zero SQL writes in the representative no-node/no-spawn gate, while dynamic node/spawn recovery remains separately owned runtime state. Runtime `player_stock` is preserved. The 2026-07-12 M2.2 closeout passes 374 relevant tests in 311.891s and the owned migration state is clean. | Generate and prove the versioned offline M2.3 content-contract artifact against both Soravelon and Builder, then repeat the Linux reload gate to close the remaining M1 lifecycle truth. |
