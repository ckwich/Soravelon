...
| Revisioned world content | **active at M2.1** | Commit `a24e231` adds a pure AST source-authority compiler with immutable operation/source-location records and fail-closed unknown-operation diagnostics. Commit `a79957d` adds literal `room_role`, `initial_room_state`, `vendor`, and `medic` operations and removes all 137 direct `.db`/`.tags` mutations from the live Python area sources. The current compiler also captures immutable literal arguments and symbolic references, rejects dynamic expressions, and deterministically compiles the 20 live zones into a semantic-hash world manifest; the redundant dynamic equipment-catalog bridge is retired in favor of `world.item_catalog`. Runtime `player_stock` is deliberately preserved rather than reset by authored vendor configuration. Startup still mutates AreaBuilder-owned runtime state; semantic graph validation, revision plan/apply records, rollback, and idempotent restart proof remain. | Add semantic validation for local/cross-zone references, exits, registry IDs, and Social taxonomy before closing M2.1. |
...
into an immutable semantic-hash manifest. Dynamic or unresolved arguments fail
closed at their source location; semantic graph validation remains before this
gate is complete.
