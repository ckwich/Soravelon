...
| Revisioned world content | **M2.1 complete; active at M2.2** | The M2.1 commit chain establishes pure literal compilation, immutable semantic manifests, complete semantic identity, fail-closed topology/registry/action validation, and destructive/player-impact classification. M2.2 now has a durable `WorldContentRevision` schema with complete lossless manifest evidence, Git/plan/lifecycle metadata, hash-integrity verification, and a database constraint permitting only one applied revision. Read-only `worldcontent validate`, `plan`, `status`, and `bootstrap-check` prove uninitialized/current/drifted/corrupt states without writes. The exact full-world bootstrap verifier reconciles zones, rooms, exits, NPCs, quests, items, registries, Social Web state, nodes, flight, and other AreaBuilder-owned runtime state to the compiled manifest; its live startup gate passes. `worldcontent bootstrap-adopt --git-commit ...` now transactionally repeats that exact proof and records the initial applied manifest, empty-to-current semantic plan, and Git provenance without mutating world content; it refuses drift, invalid provenance, or pre-existing revision authority. Runtime `player_stock` is preserved. Startup still mutates AreaBuilder-owned state; occupancy checks, locking, atomic apply/rollback, and idempotent restart proof remain. | Implement locked atomic apply with durable failure/rollback records and occupied-room protection, then remove startup mutation and prove restart/reload idempotence. |
