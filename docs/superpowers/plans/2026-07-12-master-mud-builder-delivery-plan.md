...
| Revisioned world content | **M2.1 complete; active at M2.2** | The M2.1 commit chain establishes pure literal compilation, immutable semantic manifests, complete semantic identity, fail-closed topology/registry/action validation, and destructive/player-impact classification. M2.2 now has a durable `WorldContentRevision` schema with complete lossless manifest evidence, Git/plan/lifecycle metadata, hash-integrity verification, and a database constraint permitting only one applied revision. Read-only `worldcontent validate`, `plan`, `status`, and `bootstrap-check` prove uninitialized/current/drifted/corrupt states without writes. The exact full-world bootstrap verifier now reconciles zones, rooms, exits, NPCs, quests, items, registries, Social Web state, nodes, flight, and other AreaBuilder-owned runtime state to the compiled manifest; its live startup gate passes. The proof also closed noncanonical case-folded room identities, an inactive authored node, ignored plain greetings, and incomplete node lore/override persistence. Runtime `player_stock` is preserved. Startup still mutates AreaBuilder-owned state; bootstrap adoption, occupancy checks, locking, atomic apply/rollback, and idempotent restart proof remain. | Add an explicit transactional bootstrap-adoption command that records the current exact manifest only after `bootstrap-check` passes, then implement locked atomic apply with durable failure/rollback records. |
