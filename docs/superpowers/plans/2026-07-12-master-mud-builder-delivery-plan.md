...
| Builder save, packaging, and UI rebuild | **active at B1.2** | Builder commits through `494ee9f` establish rollback-safe typed saves, persisted-byte validation, and accessible recovery errors. Commit `56c2374` adds Rust-managed canonical content-root authority. Only Rust-owned folder and Save As dialogs grant roots; native list, parse, and save reject unregistered paths before sidecar or filesystem work. RED/GREEN tests prove canonical descendants pass, sibling paths fail, and Unix symlink escapes resolve outside and fail. A frontend RED/GREEN contract proves native Save As invokes `open_area_save_dialog` and never the JavaScript dialog plugin. All 41 frontend/development tests, the production build, 42 passing sidecar tests with 15 platform skips, and all seven warning-free Rust tests pass. Generic browser-development `/sidecar` and file endpoints plus native generic template/recovery FS plugin calls remain open. Rust tests still require a test-only empty `externalBin` override because the target sidecar is absent; that remains B1.3 evidence. | Replace arbitrary browser-development sidecar dispatch with typed operation endpoints, then remove generic template/recovery file capabilities in both runtimes before closing B1.2. |
...
**Exact next slice:** add RED frontend transport contracts proving browser
development parse, validate, serialize, serialized-source validation, template
parse, and template serialization each call a fixed typed endpoint and never
post caller-controlled `subcommand`, `args`, or arbitrary stdin to `/sidecar`.
Make them GREEN with fixed server routes and delete `devSidecar` plus the generic
`/sidecar` dispatcher. Preserve generic file endpoints for the following typed
template/recovery capability slice. The user explicitly removed the Builder
