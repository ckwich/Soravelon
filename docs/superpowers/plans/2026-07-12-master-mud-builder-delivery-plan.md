...
| Builder save, packaging, and UI rebuild | **active at B1.2** | Builder commits through `41bf9f7` establish rollback-safe typed zone saves, accessible recovery, native root authority, and fixed browser sidecar routes. Commit `5d86960` removes generic recovery stat/delete APIs and endpoints. App now supplies only an authored `.py` source; Rust or browser development derives its exact `.py.tmp` sibling, checks authority, returns both mtimes, and idempotently removes only the sibling. RED/GREEN browser transport plus native characterization prove fixed commands/endpoints and no plugin stat/remove calls. Rust tests prove exact sibling derivation and reject non-`.py` or `.py.tmp` as source authority. All 43 frontend/development tests, the production build, 42 passing sidecar tests with 15 platform skips, and all eight warning-free Rust tests pass. Generic template dialog/read/write capabilities remain in browser development and native plugins. Rust tests still require a test-only empty `externalBin` override because the target sidecar is absent; that remains B1.3 evidence. | Replace template open/read/save with Rust/browser typed transactions, then remove remaining generic dialog/FS plugin and development file endpoints before closing B1.2. |
...
**Exact next slice:** add RED sidecar and transaction contracts for template
source validation from persisted bytes, then introduce typed `open`, `read`, and
`save mob templates` commands/endpoints. Rust-owned file selection must grant
the canonical parent; read/save must require that authority and the exact
`mob_templates.py` basename; save must serialize typed templates, validate the
persisted temp bytes, and use the same rollback-safe replacement semantics as
zones. Cut `MobTemplateView` to the typed save and delete generic text-file APIs
and endpoints. The user explicitly removed the Builder
