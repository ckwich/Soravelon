...
| Builder save, packaging, and UI rebuild | **active at B1.3**; B1.2 **complete** | Builder commits through `50878e8` establish typed rollback-safe zone, mob-template, and layout persistence; accessible recovery and template feedback; native root authority; fixed browser sidecar routes; and removal of generic frontend filesystem/dialog access and webview permissions. Commit `66f2311` closes the final B1.2 path gaps: native template parsing now requires the exact basename inside a user-granted canonical root; browser development limits area listing, parsing, saving, and recovery to the configured canonical `world/areas` directory, limits templates to exact `world/mob_templates.py`, and rejects symlink escapes before sidecar or filesystem access. Dead frontend shell/opener packages, direct Rust opener registration, and its webview permission are removed. All 48 frontend/development tests, the production build, 44 passing sidecar tests with 15 platform skips, and all twelve warning-free Rust tests pass. Live fixed-endpoint probes rejected four cross-root operations. `npm audit --omit=dev` still reports eight production dependency findings; Rust tests require a test-only empty `externalBin` override because target sidecars are absent. Both are now active B1.3 evidence. | Replace Windows-only sidecar discovery with a reproducible target-aware build/package/verify path, then resolve production dependency findings deliberately. |
...
**Exact next slice:** characterize the packaged sidecar naming contract for the
current host and all four launch target triples, then add a single offline build
command that creates Tauri's required `area_tools-<target>` artifact from the
pinned Python environment and verifies its filename and executable smoke path.
Delete the Windows-only manual discovery fallback only after the Tauri-managed
target-aware path is proven. The user explicitly removed the Builder
