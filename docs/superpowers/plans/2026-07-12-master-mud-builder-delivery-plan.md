...
| Builder save, packaging, and UI rebuild | **active at B1.2** | Builder commits through `7539853` establish one typed transaction with rollback-safe replacement. Commit `b460080` moves authoritative validation after the temp write in both Rust and browser development: each runtime re-reads persisted bytes and validates those bytes before any original/backup mutation. RED/GREEN corruption injection proves a writer cannot substitute invalid bytes after preflight; invalid candidates are rejected and removed while original and trusted backup remain unchanged. Partial-write fault tests prove original and backup remain unchanged, the partial candidate stays recoverable, and no validator or replacement runs. All 38 frontend/development tests, the production build, 42 passing sidecar tests with 15 platform skips, and all five warning-free Rust transaction tests pass. User-facing recovery errors still rely on alerts/status, and native path authorization plus generic browser-development capabilities remain open. Rust tests still require a test-only empty `externalBin` override because the target sidecar is absent; that remains B1.3 evidence. | Replace alert-only save failures with an accessible recoverable error surface, then constrain native and browser-development file/sidecar capabilities before closing B1.2. |
...
**Exact next slice:** add a RED frontend behavior contract proving a manual save
transaction failure produces a persistent, keyboard-dismissable, screen-reader
labelled error surface with the recovery detail, instead of relying on
`window.alert`. Make manual Save and Save As GREEN through that shared surface;
keep auto-save's compact status but expose an assistive error description. Leave
native authorized-root state and generic development-capability removal for the
following slice. The user explicitly removed the Builder
