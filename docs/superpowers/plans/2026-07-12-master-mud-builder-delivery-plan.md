...
| Production configuration hardening | **complete** for source and disposable-PostgreSQL contract; host lifecycle remains **active** | `ceea166` and `3676aa7` established environment-owned secrets, fail-closed production settings, listener checks, and `tests/test_production_settings.py`. A 2026-07-12 disposable PostgreSQL run passed empty-database migrations, repository hygiene, migration drift, Django deploy checks, smoke, reconciliation, collectstatic, and service prestart. No production deployment is claimed. | M1 still requires Linux-like supervisor lifecycle evidence and the remaining PostgreSQL concurrency/release gates. |
| Executable release verification | **active** | `df09dd8` added `scripts/verify_release.py`; `b52b50f` fixes isolated CI shards to run `full`, because Evennia initializes against the fresh base database before Django creates its test database. A sterile PostgreSQL `full --shard 2/4` passed 453 tests, including economy and quest concurrency. Shard 4 exposed a repeatable multi-second AreaBuilder room-tag lookup and was stopped cleanly rather than claimed green. The master M6 twice-clean gate has not run. | Prove and repair the PostgreSQL AreaBuilder lookup gate, then rerun shard 4 and inventory concurrency before claiming M1 data-integrity or release completion. |
