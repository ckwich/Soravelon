...
| Fresh-player, living-world integration, content quality, and release candidate | **M3 active** | M1 and M2 are complete, and B1.1 is green on the available macOS ARM package path. The compiled playable-connectivity audit models the live discovery authority. Deterministic, idempotent `discover_flight_point` rewards now let the reachable Missing Shipment and Courier Manifest quests teach Varath Prime and Tremen without moving the player or weakening origin-presence checks. Those clusters and their walking-linked zones are now reachable in the authored journey; only Korahei and its four walking-linked zones remain disconnected because `korahei_courier` has no route into the network. Existing isolated systems still do not prove the database-backed fresh-player, combined Social/co-op, launch-content, or twice-clean release gates. B1.3's full macOS Intel/Windows/Linux native matrix remains externally unavailable because Builder has no configured remote CI authority. | Decide and author the Korahei courier link, then continue the database-backed M3 journey; first-Practiced pacing also awaits product calibration. |
