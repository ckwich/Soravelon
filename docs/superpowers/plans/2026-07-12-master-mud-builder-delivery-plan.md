...
| Production configuration hardening | **complete** for source and disposable-PostgreSQL contract; host lifecycle remains **active** | `ceea166` and `3676aa7` established environment-owned secrets, fail-closed production settings, listener checks, and `tests/test_production_settings.py`. A 2026-07-12 disposable PostgreSQL run passed empty-database migrations, repository hygiene, migration drift, Django deploy checks, smoke, reconciliation, collectstatic, and service prestart. No production deployment is claimed. | M1 still requires a restored-copy rehearsal, sterile shards 1 and 3, and Linux-like supervisor lifecycle evidence. |
| Executable release verification | **active** | `df09dd8` added `scripts/verify_release.py`; `b52b50f` makes every isolated CI shard migrate its own disposable base database. Sterile PostgreSQL `full --shard 2/4` passed 453 tests, including economy and quest concurrency. Commit `3076197` adds the proven Evennia attribute lookup index, bounds exact ObjectDB tag reads without global aggregation, and makes the full-world integration test exercise production-like autocommit. Sterile `full --shard 4/4` then passed 517 tests in 1,257 seconds, including inventory concurrency and all 20 deferred cross-zone exits; the changed-surface suite passed 409 tests. The master M6 twice-clean gate has not run. | Run sterile PostgreSQL shards 1 and 3, then close the remaining restored-copy and Linux supervisor gates before claiming M1 complete. |
