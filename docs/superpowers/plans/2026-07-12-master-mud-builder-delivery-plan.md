...
| Builder save, packaging, and UI rebuild | **active at B1.2** | Builder commits through `fb757c8` establish one typed native/browser transaction with no generic zone writer. Commit `7539853` removes delete-before-replace in both Rust and browser development and adds explicit rollback staging. RED/GREEN fault injection proves a deliberate final replacement failure restores the authored original, preserves an existing trusted backup, retains the validated temp candidate, and reports the failure in both runtimes; successful saves still rotate the prior original into backup. All 36 frontend/development tests, the production build, 42 passing sidecar tests with 15 platform skips, and all three warning-free Rust transaction tests pass. Temp-write failure and validation of the bytes actually written to temp remain unproven. Rust tests still require a test-only empty `externalBin` override because the target sidecar is absent; that remains B1.3 evidence. | Add temp-write fault injection and validate the persisted temp bytes before any original/backup mutation, then close the remaining recovery/accessibility gate. |
...
**Exact next slice:** add a RED Rust corruption-injection test proving validation
runs against the bytes read back from the persisted temp candidate—not merely
the pre-write string—and that rejection leaves original and trusted backup
unchanged. Make it GREEN, then mirror the same real-filesystem invariant in
browser development. Include a partial temp-write failure assertion while the
write seam is available. Leave user-facing recovery-message accessibility and
capability-root hardening for the following slice. The user explicitly removed the Builder
