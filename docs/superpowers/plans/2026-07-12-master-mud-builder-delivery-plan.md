...
| Builder save, packaging, and UI rebuild | **active at B1.3**; B1.2 **complete** | Builder commits through `66f2311` complete B1.2 with typed rollback-safe persistence, accessible failures, canonical path authority, fixed development routes, and no generic webview file/dialog/shell surface. Commit `bd18461` establishes the B1.3 native packaging spine: Python 3.13.13 and the full PyInstaller 6.21.0 build/test environment are pinned; one cross-platform npm command detects macOS ARM/Intel, Windows x64, or Linux x64 and refuses cross-target relabeling; generated Tauri target artifacts are executable-smoked before use; `npm run verify` rebuilds the current target first; and both file-argument and stdin sidecar operations now use Tauri-managed discovery with the Windows-only fallback deleted. On macOS ARM, 48 frontend, 51 sidecar with 15 platform skips, and 13 Rust tests pass; a debug `.app` bundle contains a real Mach-O arm64 `Contents/MacOS/area_tools` that passes the parser smoke. Misclassified `shadcn` tooling moved to development dependencies and safe transitive fixes leave both full and production npm audits at zero. Other platform artifacts are contract-tested but not yet built on their native runners. | Add native-runner CI for all four target triples and a packaged save/reopen smoke before closing B1.3. |
...
**Exact next slice:** add a native-runner CI matrix for macOS ARM, macOS Intel,
Windows x64, and Linux x64 that installs the pinned toolchains, runs
`npm run verify`, builds the target-specific Tauri bundle, and verifies the
embedded sidecar architecture/name. Add a deterministic packaged save/reopen
smoke that exercises the installed command path against a temporary supported
area without touching Soravelon source. Keep platform results distinct: local
macOS ARM evidence does not prove the other three runners. The user explicitly
removed the Builder GSD-workflow requirement for this program; continue with
the repository's tests and scoped commit discipline.
