...
| Builder save, packaging, and UI rebuild | **active at B1.2** | Builder commits through `56c2374` establish rollback-safe typed saves, accessible recovery, and native canonical-root authority. Commit `41bf9f7` replaces browser-development caller-controlled `/sidecar` dispatch with six fixed typed routes for zone parse/validate/serialize/source-validation and template parse/serialize; `devSidecar` and the generic dispatcher are deleted. RED/GREEN transport tests prove Social collections and remaining operations send typed bodies without subcommand, args, or arbitrary stdin. Live HTTP proof returns 404 for `/sidecar` while typed `/parse-zone` parses Vael's Crossing successfully. All 42 frontend/development tests, the production build, 42 passing sidecar tests with 15 platform skips, and all seven warning-free Rust tests pass. Generic file read/write/stat/remove capabilities remain in browser development and native plugins. Rust tests still require a test-only empty `externalBin` override because the target sidecar is absent; that remains B1.3 evidence. | Replace recovery stat/remove with typed root-authorized commands/endpoints, then replace template open/read/save and remove generic dialog/FS plugin capabilities before closing B1.2. |
...
**Exact next slice:** add RED frontend/native contracts for typed recovery stat
and removal: only an authorized `.py` source and its exact `.py.tmp` sibling may
be inspected, and only that recovery sibling may be removed. Make them GREEN
with Rust commands behind `ContentRootAuthority` and matching fixed browser-dev
routes; delete generic `statEditorFile`/`removeEditorFile` plugin use and generic
`/stat-file`/`/remove-file` endpoints. Leave template dialog/read/save typing for
the following slice. The user explicitly removed the Builder
