...
| Revisioned world content | **active at M2.1** | Commit `a24e231` adds a pure AST source-authority compiler; `a79957d` removes all 137 direct runtime mutations; `e42728e` adds immutable literal/reference capture and a deterministic 20-zone semantic manifest; `4d6d855` validates topology, explicit one-way intent, and Social references. The compiler now also validates globally stable IDs, mob/item/material registries, quest graphs/givers/objective targets, reward/trigger action types, and skill IDs. This proof exposed and repaired unreachable `visit` progress plus room-target delivery semantics while preserving required carried-item consumption. Runtime `player_stock` is preserved. Startup still mutates AreaBuilder-owned state; destructive-change policy, revision plan/apply records, rollback, and idempotent restart proof remain. | Add destructive-change classification/policy to the immutable definition graph, then close M2.1 and begin M2.2. |
...
**Current evidence:** all 20 live literal zones compile without diagnostics
into an immutable semantic-hash manifest. Dynamic expressions, unresolved room
and Social references, ambiguous directions, and unstated one-way exits fail
closed at their source locations. Runtime registry, quest graph/objective, and
action identifiers are also proven. Destructive-change classification remains
