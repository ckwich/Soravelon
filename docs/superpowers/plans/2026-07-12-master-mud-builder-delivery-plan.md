...
| Builder save, packaging, and UI rebuild | **active at B1.2** | Builder commits through `b460080` establish one typed transaction with rollback-safe replacement and persisted-byte validation. Commit `494ee9f` replaces manual Save/Save As `window.alert` failures with a persistent `alertdialog` carrying the full recovery detail, initial focus on Dismiss, button dismissal, and Escape dismissal; auto-save failures now use an assertive live region with retry/recovery guidance. RED/GREEN server-rendered semantic tests pass. A live Playwright fault injection against Vael's Crossing proved one labelled/described alert dialog, exact recovery text, focused dismissal, Escape closure, and an unchanged source SHA-256. An initial un-intercepted smoke save was detected by hash and fully restored from its exact-hash transaction backup with no artifact left. All 40 frontend/development tests, the production build, 42 passing sidecar tests with 15 platform skips, and all five warning-free Rust transaction tests pass. Native path authorization plus generic browser-development capabilities remain open. Rust tests still require a test-only empty `externalBin` override because the target sidecar is absent; that remains B1.3 evidence. | Constrain native saves/reads to roots granted by Rust-owned folder/save dialogs, then replace generic browser-development file/sidecar capabilities with typed endpoints. |
...
**Exact next slice:** add RED Rust authorization tests proving `save_zone_file`
rejects paths outside registered content roots and accepts a descendant of a
root granted by a Rust-owned folder or Save As dialog. Make them GREEN with
managed canonical roots and path checks before sidecar or filesystem work, then
route native dialog clients through those authority-granting commands. Preserve
browser development unchanged for this slice; replace its generic capabilities
in the following slice. The user explicitly removed the Builder
