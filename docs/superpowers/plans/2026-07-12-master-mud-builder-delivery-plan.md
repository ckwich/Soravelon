...
| Fresh-player, living-world integration, content quality, and release candidate | **M3 unblocked** | M1 and M2 are complete, and B1.1 is green on the available macOS ARM package path. Existing isolated systems still do not prove the database-backed fresh-player, combined Social/co-op, launch-content, or twice-clean release gates. B1.3's full macOS Intel/Windows/Linux native matrix remains externally unavailable because Builder has no configured remote CI authority. | Begin the smallest database-backed M3 fresh-player journey slice while preserving the offline Builder boundary. |
