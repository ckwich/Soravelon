...
| Builder save, packaging, and UI rebuild | **active at B1.2** | Builder commits through `445a517` establish the typed native/browser manual-save transaction. Commit `fb757c8` cuts auto-save and World Map reverse-exit export to typed `saveZoneFile` calls, deletes the public `writeZoneFile` API, Rust `write_zone_file` command, and browser `/write-zone-file` endpoint, and reconciles Builder guidance. A RED/GREEN public-surface contract plus TypeScript compilation proves no production caller can retain the generic writer. All 35 frontend/development tests, the production build, 42 passing sidecar tests with 15 platform skips, and both warning-free Rust transaction tests pass. Validation failure is proven safe, but current native and development replacement helpers still remove the destination before rename; deliberate replacement/write failure recovery is not yet proven. Rust tests still require a test-only empty `externalBin` override because the target sidecar is absent; that remains B1.3 evidence. | Prove and harden replacement failure first, then add temp-write failure and post-write validation gates before closing B1.2. |
...
**Exact next slice:** add a RED Rust fault-injection test proving a deliberate
final replacement failure preserves the original destination, its trusted
backup, and a recoverable temp candidate. Make it GREEN by removing the
delete-before-rename window and adding explicit rollback semantics; mirror the
same invariant in the browser-development transaction with a real temporary
filesystem test. Leave temp-write failure and post-write validation for the
