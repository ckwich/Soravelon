...
| Builder save, packaging, and UI rebuild | **active at B1.2** | Builder commit `47ceb98` adds a typed native `save_zone_file` transaction that owns sidecar serialization, serialized-source validation, and atomic replacement. Its injected validation-failure test proves the original, an existing backup, and temp state remain unchanged. Commit `e1987ad` adds the typed `saveZoneFile(path, zone)` client and cuts native manual Save/Save As to that transaction while retaining the structured frontend preflight for the Problems panel; its RED/GREEN IPC contract proves the typed model reaches `save_zone_file`, never the legacy writer. All 32 frontend tests, the production build, and both Rust transaction tests pass. Browser-dev and auto-save still use legacy writes, and Rust tests still require a test-only empty `externalBin` override because the real macOS sidecar artifact is missing; that remains B1.3 evidence. | Put browser-dev behind the same typed save contract, then cut auto-save and prove serialization, temp-validation, backup, replacement, and recovery failures before removing legacy writers. |
...
**Exact next slice:** add a RED browser-dev contract proving `saveZoneFile(path,
zone)` sends the typed model to a single typed development endpoint rather than
calling the generic serialized-content writer or arbitrary sidecar endpoint.
Make it GREEN with an equivalent server-owned serialize, validate, and atomic
replacement transaction, then route browser-dev manual Save/Save As through the
same TypeScript API. Leave auto-save and legacy-path removal for the following
