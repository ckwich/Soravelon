...
| Builder Social DSL round trip | **active and unsafe to save** | Builder commits `8170609`, `3b7d26a`, and `d712ec5` prove the live-source contract, fail-closed unknown literal `area.*` rejection with source location, and Python sidecar parse/serialize preservation for `social_node`/`social_edge`. `4704138` and `6e18e02` prove typed Social collections plus lossless store normalization/snapshots. `0c7572e` proves browser-dev and native Tauri IPC preservation; `05149f6` proves malformed Social topology is rejected offline with the same taxonomy and errors as the live Soravelon validator. Focused frontend and sidecar gates pass. The broad live-area sidecar run is not green: the dirty `ashreach_plains.py` currently violates practice references, stale constant extraction expects a removed `ALL_DOMAINS` literal, and the equipment-catalog test still contradicts the explicit dynamic-helper/read-only policy. The unrelated deleted `src-tauri/binaries/.gitkeep` remains preserved. | Align the equipment-catalog contract to explicit unsupported/read-only behavior and add an actual supported Social source save/reopen proof; do not claim B1.1 complete while the broad live-area gate fails. |
...
**Exact next slice:** reconcile the dynamic `equipment_catalog.py` test with the
approved explicit unsupported/read-only contract, then prove a supported Social
area survives the actual local sidecar serialize/validate/reparse path. The gate
must distinguish supported literal AreaBuilder files from dynamic helper modules
instead of advertising both as editable. Do not weaken unknown-call rejection,
touch the dirty Ashreach content, add Social UI, or widen into B1.2 atomic writes.
The user explicitly removed the Builder GSD-workflow requirement for this
