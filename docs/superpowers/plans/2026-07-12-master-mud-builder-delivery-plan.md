...
| Revisioned world content | **active at M2.1** | Commit `a24e231` adds a pure AST source-authority compiler with immutable operation/source-location records and fail-closed unknown-operation diagnostics. Commit `a79957d` removes all 137 direct `.db`/`.tags` mutations from live area sources. Commit `e42728e` captures immutable literal arguments/references and deterministically compiles the 20 live zones into a semantic-hash manifest. The compiler now also rejects unresolved local/cross-zone rooms, duplicate exit directions, implicit nonreciprocal exits, and invalid or unresolved Social topology; every live nonreciprocal exit carries explicit `one_way=True` intent. Runtime `player_stock` is preserved. Startup still mutates AreaBuilder-owned runtime state; registry/action ID validation, revision plan/apply records, rollback, and idempotent restart proof remain. | Validate registry/action IDs and destructive-change policy before closing M2.1. |
...
closed at their source locations. Registry/action ID and destructive-change
validation remain before this gate is complete.
