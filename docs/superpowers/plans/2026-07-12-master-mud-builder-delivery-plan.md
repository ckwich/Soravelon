...
| Builder save, packaging, and UI rebuild | **active at B1.2** | Builder commits through `5d86960` establish rollback-safe typed zone saves, accessible recovery, native root authority, fixed browser sidecar routes, and typed recovery access. Commit `c7e73f7` adds typed mob-template open/read/save operations in native and browser-dev modes: Rust owns file selection and canonical-root grants, the exact `mob_templates.py` basename is enforced, serialization is internal to the transaction, persisted temp bytes are parsed before replacement, and the generic text-file/template serializer bridges are gone. All 44 frontend/development tests, the production build, 44 passing sidecar tests with 15 platform skips, and all ten warning-free Rust tests pass. The remaining broad file capability is canvas layout persistence through the generic filesystem plugin; template save feedback also still uses blocking browser alerts. Rust tests require a test-only empty `externalBin` override because the target sidecar is absent; that remains B1.3 evidence. | Move canvas layout persistence behind typed Rust/browser operations, remove the generic filesystem plugin and broad permissions, then make template save feedback accessible before closing B1.2. |
...
**Exact next slice:** characterize canvas layout persistence, then replace its
generic filesystem-plugin reads and writes with typed Rust commands and fixed
browser-development endpoints constrained to the app-owned layout location.
Remove the frontend and Rust filesystem plugin dependencies and broad filesystem
capabilities once no caller remains. Replace `MobTemplateView`'s blocking save
alerts with an accessible status/error surface, and prove both paths through
focused tests. The user explicitly removed the Builder GSD-workflow requirement
for this program; continue with the repository's tests and scoped commit
discipline.
