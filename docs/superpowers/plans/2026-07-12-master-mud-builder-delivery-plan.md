...
| Builder save, packaging, and UI rebuild | **active at B1.2** | Builder commits through `c7e73f7` establish typed rollback-safe zone and mob-template persistence, accessible recovery, native root authority, and fixed browser sidecar routes. Commit `50878e8` moves canvas layout state behind typed Rust commands rooted in Tauri app data and fixed browser-development endpoints rooted in editor app data; both validate identifiers and coordinate shape and use persisted-byte validation before atomic replacement. Frontend generic filesystem/dialog packages, the direct Rust filesystem dependency, and every webview filesystem/dialog permission are removed. Template open/save feedback is now a non-blocking polite status or assertive alert instead of browser alerts. All 47 frontend/development tests, the production build, 44 passing sidecar tests with 15 platform skips, and all eleven warning-free Rust tests pass. A live fixed-endpoint probe round-tripped layout state and rejected traversal. `npm audit --omit=dev` still reports eight production dependency findings; that remains an explicit B1.3 gate. Rust tests still require a test-only empty `externalBin` override because the target sidecar is absent. | Close the remaining typed-path authority gaps in native template parsing and browser-development area listing/parsing before declaring B1.2 complete. |
...
**Exact next slice:** add RED authority tests proving native template parsing
rejects an ungranted path and browser development cannot list, parse, save, or
inspect recovery outside the configured `world/areas` root. Require the exact
`world/mob_templates.py` path for browser template parsing, then re-inventory
every path-bearing command and fixed endpoint. If no broader path surface
remains, rerun the B1.2 failure-preservation/accessibility gates and mark B1.2
complete. The user explicitly removed the Builder GSD-workflow requirement for
